...
export default function PriceOptimizationFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      <Helmet>
        <link rel="canonical" href="https://insydz.com/features/price-optimization-feature" />
        <title>AI Price Optimization Software for Amazon | Insydz</title>
        <meta name="description" content="Maximise margins with Insydz's AI price optimization software. Set dynamic pricing rules, win the Buy Box, and stop leaving money on the table. Try free." />
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0" />
         <script
    type="application/ld+json"
    dangerouslySetInnerHTML={{
      __html: JSON.stringify(schemaSoftwarePrice),
    }}
  />
  ...
  <script
    type="application/ld+json"
    dangerouslySetInnerHTML={{
      __html: JSON.stringify(schemaBreadcrumbPrice),
    }}
  />
  ...
  <script
    type="application/ld+json"
    dangerouslySetInnerHTML={{
      __html: JSON.stringify(schemaFAQPrice),
    }}
  />        
      </Helmet>
            ...
            <div>
              ...
              <ul className="space-y-2 sm:space-y-3">
                {[
                  ...
                  <li key={i}>
                    <button onClick={() => setLocation(item.route)} className="text-xs sm:text-sm text-gray-400 hover:text-white transition-colors text-left">{item.label}</button>
                  </li>
                ))}
              </ul>
            </div>
