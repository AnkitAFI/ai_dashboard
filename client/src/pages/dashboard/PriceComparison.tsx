...
// import {
//   ArrowLeft, Lock, Crown, Star, TrendingUp, TrendingDown,
//   Minus, Target, BarChart2, RefreshCw, Menu, X,
//   Zap, CheckCircle, ShieldCheck, Package,
// } from "lucide-react";
...
// // ── Stat Card ─────────────────────────────────────────────────────────────────
// function StatCard({ label, value, sub, accent = false, locked = false, tier }: {
//   label: string; value: string; sub?: string;
//   accent?: boolean; locked?: boolean; tier?: "basic" | "premium";
// }) {
//   return (
//     <div className="relative bg-white rounded-2xl p-4 border border-slate-100 shadow-sm overflow-hidden">
//       {locked && tier && <TierGate tier={tier} feature={label} />}
//       <p className="text-xs text-slate-400 font-medium mb-1">{label}</p>
//       <p className={`text-xl font-black ${accent ? "text-sky-700" : "text-slate-800"}`}>{value}</p>
...
//               </h2>
//               <p className="text-xs text-slate-500 hidden sm:block">Benchmark your pricing against the market</p>
//             </div>
//           </div>
...
//                 <StatCard
//                   label="Current Price"
//                   value={fmt(data.current_price, currency)}
//                   sub={data.currency}
//                   accent
//                 />
//                 <StatCard
//                   label="Original / MRP"
//                   value={fmt(data.original_price, currency)}
//                 />
//                 <StatCard
//                   label="Discount"
//                   value={data.discount_pct != null ? `${data.discount_pct}%` : "—"}
//                   sub="off MRP"
//                 />
//                 <StatCard
//                   label="No. of Offers"
//                   value={data.num_offers != null ? String(data.num_offers) : "—"}
//                   sub="competing sellers"
//                 />
//               </div>
...
//               <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
//                 <div className="relative bg-white rounded-2xl p-4 border border-slate-100 shadow-sm overflow-hidden">
//                   {!isBasic && <TierGate tier="basic" feature="Min Offer Price" />}
//                   <p className="text-xs text-slate-400 font-medium mb-1">Min Offer Price</p>
//                   <p className="text-xl font-black text-slate-800">{fmt(data.min_offer_price, currency)}</p>
//                   <p className="text-xs text-slate-400 mt-0.5">lowest competing seller</p>
//                 </div>
//                 <div className="relative bg-white rounded-2xl p-4 border border-slate-100 shadow-sm overflow-hidden">
//                   {!isBasic && <TierGate tier="basic" feature="Market Average" />}
//                   <p className="text-xs text-slate-400 font-medium mb-1">Market Average</p>
//                   <p className="text-xl font-black text-slate-800">{fmt(data.market_avg, currency)}</p>
//                   <p className="text-xs text-slate-400 mt-0.5">{data.competitor_count ?? "—"} products</p>
//                 </div>
//                 <div className="relative bg-white rounded-2xl p-4 border border-slate-100 shadow-sm overflow-hidden">
//                   {!isBasic && <TierGate tier="basic" feature="Price Range" />}
//                   <p className="text-xs text-slate-400 font-medium mb-1">Market Range</p>
//                   <p className="text-sm font-black text-slate-800">
//                     {fmt(data.market_min, currency)} – {fmt(data.market_max, currency)}
//                   </p>
//                   <p className="text-xs text-slate-400 mt-0.5">min – max</p>
//                 </div>
//                 <div className="relative bg-white rounded-2xl p-4 border border-slate-100 shadow-sm overflow-hidden">
//                   {!isBasic && <TierGate tier="basic" feature="Price Position" />}
//                   <p className="text-xs text-slate-400 font-medium mb-1">Your Position</p>
//                   {data.price_position
//                     ? <PositionBadge position={data.price_position} />
//                     : <p className="text-xl font-black text-slate-800">—</p>
//                   }
//                   {data.price_percentile != null && (
//                     <p className="text-xs text-slate-400 mt-1">top {data.price_percentile}% of market</p>
//                   )}
//                 </div>
//               </div>
...
//                     <p className="text-xs text-slate-400 mt-0.5">How your price compares to market min, avg & max</p>
//                   </div>
...
//                       <YAxis
//                         tickFormatter={(v) => (currency === "INR" ? `₹${(v/1000).toFixed(0)}k` : `$${v}`)}
...
//                   Green = less competition · Red = crowded · Blue = your price band
//                 </p>
...
//                         <span className="text-xs text-slate-600 w-32 shrink-0 font-mono">{b.label}</span>
...
//                           style={{ color: b.your_price_in_band ? "#0ea5e9" : densityColor[b.density] }}
//                         >
...
//                   </div>
//                   <div>
//                     <p className="font-bold text-slate-800 text-sm mb-1">AI Pricing Recommendation</p>
...
//                       <button
//                         key={i}
//                         onClick={() => navigate(`/seller/price-comparison?asin=${p.asin}&seller_id=${sellerId}`)}
...
//                           {p.photo
//                             ? <img src={p.photo} alt={p.title} className="w-full h-full object-contain p-0.5" />
...
//                       {!isBasic
//                         ? "Get market benchmarks, price band density, and AI pricing tips — Basic · ₹1,999/mo"
//                         : "Get AI pricing recommendations and cross-product comparisons — Premium · ₹2,999/mo"}
//                     </p>
//                     <div className="flex flex-wrap gap-3 mt-2">
//                       {(!isBasic
//                         ? ["Market benchmarks", "Price band density", "Percentile ranking", "AI pricing tip"]
//                         : ["AI pricing tip", "Portfolio comparison", "Price trend alerts"]
//                       ).map((f) => (
