// // src/pages/ProfitabilityOptimizer.tsx
...
//     return (
//       <div className="mt-3 min-h-16 max-h-80 overflow-y-auto bg-slate-950 rounded-xl p-4 font-mono text-xs text-green-300 leading-relaxed whitespace-pre-wrap border border-slate-800">
//         {stream.streaming && !stream.text && <span className="text-slate-500 animate-pulse">llama3.2:3b is thinking...</span>}
//         <div className="prose prose-sm text-slate-200 whitespace-pre-line">
//   <ReactMarkdown>
//     {stream.text}
//   </ReactMarkdown>
// </div>
//         {stream.streaming && stream.text && <span className="animate-pulse">▌</span>}
//         {stream.error === "upgrade_required" && <span className="text-amber-400">⚠ Upgrade required.</span>}
//         {stream.error === "ollama_offline" && <span className="text-red-400">⚠ Ollama offline. Run: <code className="bg-red-950 px-1 rounded">ollama serve</code></span>}
//         {stream.error && !["upgrade_required","ollama_offline"].includes(stream.error) && <span className="text-red-400">⚠ {stream.error}</span>}
//       </div>
//     );
//   }
...
//   </code>
// )}
//           {ready && <span className="text-xs text-green-400/70">local · private · no API cost</span>}
//         </div>
//       </div>
...
//                 activeMode === m.id ? "bg-violet-600 text-white border-violet-500 shadow-lg shadow-violet-900/30"
//                 : locked ? "bg-slate-900/50 text-slate-600 border-slate-800 cursor-not-allowed"
...
//             <div>
//               <p className="text-sm font-medium text-slate-200">Full profitability analysis</p>
//               <p className="text-xs text-slate-500 mt-0.5">4 recommendations from your exact numbers</p>
//             </div>
//             <div className="flex gap-2">
//               {analyzeStream.text && <button onClick={analyzeStream.reset} className="text-xs text-slate-500 hover:text-slate-300 transition-colors">Clear</button>}
//               <button onClick={runAnalyze} disabled={analyzeStream.streaming || !calcResult || !ready}
//                 className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-xs rounded-xl font-medium transition-all">
...
//           ) : (
//             <div className="mt-3 bg-slate-950/40 rounded-xl p-5 text-center border border-slate-800/40">
//               <p className="text-slate-600 text-xs">Click "Run analysis" for AI recommendations</p>
//             </div>
//           )}
//         </div>
//       )}
...
//                 <p className="text-slate-500 text-xs mb-3">Ask anything about your product's numbers</p>
//                 <div className="flex flex-wrap gap-2 justify-center">
//                   {QUICK_Q.map((q) => (
//                     <button key={q} onClick={() => setChatInput(q)}
//                       className="text-xs px-3 py-1.5 bg-slate-800 text-slate-400 rounded-full hover:bg-violet-800 hover:text-white transition-colors border border-slate-700">{q}</button>
//                   ))}
//                 </div>
//               </div>
//             )}
...
//                 <div className={`max-w-[85%] px-3.5 py-2.5 text-xs leading-relaxed whitespace-pre-wrap rounded-2xl ${
//                   msg.role === "user" ? "bg-violet-700 text-white rounded-br-sm" : "bg-slate-800 text-green-300 rounded-bl-sm font-mono border border-slate-700"}`}>
//                   <div className="prose prose-sm text-green-300">
//                   <ReactMarkdown>
//   {String(msg.content ?? "")}
// </ReactMarkdown>
// </div>
...
//                 <div className="max-w-[85%] px-3.5 py-2.5 text-xs font-mono bg-slate-800 text-green-300 rounded-2xl rounded-bl-sm border border-slate-700 leading-relaxed whitespace-pre-wrap">
//                   {chatStream.text ? (
                    
//   <ReactMarkdown className="prose prose-sm text-green-300">
//     {chatStream.text}
//   </ReactMarkdown>
// ) : (
//   <span className="animate-pulse text-slate-500">▌</span>
// )}
//                   {chatStream.streaming && chatStream.text && <span className="animate-pulse">▌</span>}
//                 </div>
//               </div>
//             )}
//             <div ref={chatEndRef} />
...
//               className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-violet-500 transition-colors" />
//             <button onClick={runChat} disabled={chatStream.streaming || !chatInput.trim() || !ready}
//               className="px-3 py-2.5 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-800 disabled:text-slate-600 text-white rounded-xl transition-all">
//               <Send className="w-3.5 h-3.5" />
//             </button>
//             {chatHistory.length > 0 && (
//               <button onClick={() => { setChatHistory([]); chatStream.reset(); setPendingChat(null); }}
//                 className="px-3 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-xl text-xs transition-colors">
...
//           <div className="flex items-start justify-between mb-2">
//             <div>
//               <p className="text-sm font-medium text-slate-200">Which strategy should I pick?</p>
//               <p className="text-xs text-slate-500 mt-0.5">AI compares your 4 scenarios and recommends one</p>
//             </div>
//             <button onClick={runScenario} disabled={scenarioStream.streaming || !scenarios || !ready}
//               className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-xs rounded-xl font-medium transition-all">
//               {scenarioStream.streaming ? "Thinking..." : "✦ Get advice"}
//             </button>
//           </div>
//           {!scenarios && <p className="mt-3 text-xs text-slate-600 bg-slate-950/40 rounded-xl p-4 border border-slate-800/40">Open the Scenarios tab first to generate data.</p>}
//           {(scenarioStream.text || scenarioStream.streaming || scenarioStream.error) && <StreamBox stream={scenarioStream} />}
//         </div>
//       )}
...
//           <div className="flex items-start justify-between mb-2">
//             <div>
//               <p className="text-sm font-medium text-slate-200">5-step health improvement plan</p>
//               <p className="text-xs text-slate-500 mt-0.5">AI turns your health score into a prioritized action plan</p>
//             </div>
//             <button onClick={runHealth} disabled={healthStream.streaming || !healthData || !ready}
//               className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-xs rounded-xl font-medium transition-all">
//               {healthStream.streaming ? "Thinking..." : "✦ Build plan"}
//             </button>
//           </div>
//           {!healthData && <p className="mt-3 text-xs text-slate-600 bg-slate-950/40 rounded-xl p-4 border border-slate-800/40">Open the Business Health tab first.</p>}
//           {(healthStream.text || healthStream.streaming || healthStream.error) && <StreamBox stream={healthStream} />}
//         </div>
//       )}
//     </div>
//   );
...
//   const [savedProducts, setSaved]     = useState<{ id: number; name: string; margin: number; profit: number; monthly: number }[]>([]);
...
//   useEffect(() => { fetchCategories(inputs.marketplace); if (userId) fetchTierInfo(); }, [userId]);
...
//     setActiveTab(tab);
//     if (tab === "scenario" && !scenarios) fetchScenarios();
//     if (tab === "market"   && !marketIntel) fetchMarketIntel();
//     if (tab === "health"   && !healthData) fetchHealth();
//   };
...
//     if (savedProducts.length >= saveLimit) { toast("Limit reached", `Upgrade to save more than ${saveLimit} products.`, "error"); return; }
//     setSaveModal(true);
//   };

//   const confirmSave = () => {
//     if (!saveName.trim() || !calcResult) return;
//     setSaved((p) => [...p, { id: Date.now(), name: saveName.trim(), margin: Number(calcResult.net_margin_pct ?? 0), profit: Number(calcResult.profit_per_unit ?? 0), monthly: Number(calcResult.monthly_profit ?? 0) }]);
//     setSaveModal(false);
//     setSaveName("");
//     toast("Saved!", `"${saveName}" added to your products.`, "success");
//   };
...
//             <h3 className="text-lg font-bold text-slate-900 mb-3">Save product</h3>
...
//               onKeyDown={(e) => e.key === "Enter" && confirmSave()} autoFocus />
//             <div className="flex gap-3">
//               <Button variant="outline" className="flex-1" onClick={() => setSaveModal(false)}>Cancel</Button>
//               <Button className="flex-1 bg-blue-600 text-white" onClick={confirmSave} disabled={!saveName.trim()}>Save</Button>
//             </div>
//           </div>
//         </div>
//       )}
...
//             <div className="flex justify-end p-4"><button onClick={() => setMobileMenu(false)}>✕</button></div>
//             <Sidebar />
//           </aside>
//         </>
//       )}
...
//           <div className="flex items-center gap-3">
//             <button onClick={() => setMobileMenu(true)} className="lg:hidden p-2 rounded-lg hover:bg-sky-100"><span className="text-xl font-bold">☰</span></button>
//             <div>
//               <h2 className="text-2xl font-bold text-sky-900">Profitability Optimizer</h2>
//               <p className="text-slate-500 text-sm mt-0.5">Real margins · Live market data</p>
//             </div>
//           </div>
//           <div className="flex items-center gap-3">
...
//               <Card className="lg:col-span-2 bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl shadow-lg">
...
//                           {m.locked && (
//                             <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] flex items-center justify-center cursor-pointer rounded-2xl"
...
//                     <Card className="bg-white/80 border border-slate-200 rounded-2xl shadow-lg relative overflow-hidden">
...
//                     <CardHeader className="pb-2">
//                       <div className="flex items-center justify-between">
//                         <CardTitle className="text-sm flex items-center gap-2"><Bookmark className="w-4 h-4 text-blue-500" /> Saved products</CardTitle>
//                         <span className="text-xs text-slate-400">{savedProducts.length}/{saveLimit === 9999 ? "∞" : saveLimit}</span>
//                       </div>
//                     </CardHeader>
...
//                           <div>
//                             <p className="text-sm font-medium text-slate-800">{String(p.name)}</p>
//                             <p className="text-xs text-slate-400">{pct(p.margin)} margin · {inr(p.profit)}/unit</p>
//                           </div>
//                           <p className="text-sm font-semibold text-green-600">{inr(p.monthly)}/mo</p>
//                         </div>
//                       ))}
//                     </CardContent>
//                   </Card>
...
//               {tabLoading && <Card className="bg-white/70 rounded-2xl shadow-lg"><CardContent className="p-12 flex flex-col items-center gap-3"><RefreshCw className="w-8 h-8 text-blue-500 animate-spin" /><p className="text-slate-500 text-sm">Building scenarios...</p></CardContent></Card>}
//               {!tabLoading && scenarios && (
//                 <>
//                   <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
//                     {(scenarios as Record<string, unknown>[]).map((s, i) => (
//                       <Card key={i} className="bg-white/80 border border-slate-200 rounded-2xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all">
...
//           {activeTab === "market" && (
//             <div className="space-y-5">
//               {tabLoading && <Card className="bg-white/70 rounded-2xl shadow-lg"><CardContent className="p-12 flex flex-col items-center gap-3"><RefreshCw className="w-8 h-8 text-blue-500 animate-spin" /><p className="text-slate-500 text-sm">Fetching live market data from your DB...</p></CardContent></Card>}
...
//           {activeTab === "health" && (
//             <div className="space-y-5">
//               {tabLoading && <Card className="bg-white/70 rounded-2xl shadow-lg"><CardContent className="p-12 flex flex-col items-center gap-3"><RefreshCw className="w-8 h-8 text-blue-500 animate-spin" /><p className="text-slate-500 text-sm">Computing health score...</p></CardContent></Card>}
...
//     } catch {
//       // silent — don't block the UI if this fails
//     } finally {
...
//     } catch {
//       toast("Delete failed", "Could not remove product.", "error");
//     }
//   };
...
//                 {/* Saved products card (DB-backed) */}
//                 {(savedProducts.length > 0 || loadingSaved) && (
//                   <Card className="bg-white/80 border border-slate-200 rounded-2xl shadow-lg">
//                     <CardHeader className="pb-2">
//                       <div className="flex items-center justify-between">
//                         <CardTitle className="text-sm flex items-center gap-2">
...
//                           Saved products
//                           {loadingSaved && <RefreshCw className="w-3 h-3 text-slate-400 animate-spin" />}
//                         </CardTitle>
//                         <span className="text-xs text-slate-400">
//                           {savedProducts.length}/{saveLimit === 9999 ? "∞" : saveLimit}
//                         </span>
//                       </div>
//                     </CardHeader>
//                     <CardContent className="pt-0 space-y-2">
...
//                       )}
//                       {savedProducts.map((p) => (
//                         <div key={p.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0 group">
//                           <div className="flex-1 min-w-0 mr-3">
//                             <p className="text-sm font-medium text-slate-800 truncate">{String(p.name)}</p>
//                             <p className="text-xs text-slate-400">
//                               {pct(p.net_margin_pct)} margin · {inr(p.profit_per_unit)}/unit
//                             </p>
...
//                             <p className="text-sm font-semibold text-green-600">{inr(p.monthly_profit)}/mo</p>
//                             <button
