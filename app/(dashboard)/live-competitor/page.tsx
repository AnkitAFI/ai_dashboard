...
import { Lock, Crown, FileSpreadsheet, UploadCloud, AlertCircle, Save, Folder } from "lucide-react";
...
function TierGate({ tier, feature }: { tier: "basic" | "premium" | "enterprise"; feature: string }) {
  ...
  return (
    <div className="absolute inset-0 bg-background/95 backdrop-blur-sm rounded-2xl flex flex-col items-center justify-center z-10 gap-3 min-h-[500px]">
      <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm ${tier === "enterprise" ? "bg-fuchsia-100 text-fuchsia-800 border border-fuchsia-300 dark:bg-fuchsia-950/50 dark:text-fuchsia-400 dark:border-fuchsia-800" : tier === "premium" ? "bg-blue-50 dark:bg-blue-950/40" : "bg-amber-50 dark:bg-amber-950/40"}`}>
        <Lock className={`w-6 h-6 ${tier === "enterprise" ? "text-fuchsia-800 dark:text-fuchsia-400" : tier === "premium" ? "text-blue-500 dark:text-blue-400" : "text-amber-500 dark:text-amber-400"}`} />
      </div>
      <div className="text-center px-6">
        <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">{feature} is locked</p>
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Available on Enterprise · Contact Sales</p>
      </div>
      <button
        onClick={() => router.push("/subscription")}
        className="flex items-center gap-2 px-6 py-2.5 rounded-full text-sm font-bold text-white shadow-md transition-all hover:scale-105 bg-fuchsia-100 text-fuchsia-800 border border-fuchsia-300 dark:bg-fuchsia-950/50 dark:text-fuchsia-400 dark:border-fuchsia-800"
      >
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Find indices of competitor columns
        const compIndices: number[] = [];
        for (let i = 1; i <= 10; i++) {
          const idx = headers.indexOf(`competitor-${i}`);
          if (idx !== -1) compIndices.push(idx);
        }
        ...
        for (let i = headerRowIdx + 1; i < rawData.length; i++) {
          ...
          for (const compIdx of compIndices) {
            ...
            if (comp && typeof comp === 'string' && comp.trim()) {
              // Only add valid 10-character competitor ASINs
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 relative">
      ...
      {showSaveModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Save ASIN List</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Enter a name to save this exact configuration of ASINs so you can quickly reload it tomorrow.</p>
            <input 
              type="text" 
              autoFocus
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2">
          Import your ASIN list and fetch real-time pricing, buy box status, and delivery timelines directly from the marketplace.
        </p>
      </div>
      ...
      {isEnterprise && (
        <>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl shadow-sm">
            <h3 className="font-semibold text-slate-800 dark:text-slate-200 mb-4">Import ASIN Data</h3>
            <div className="flex items-start gap-6">
              <label className="flex flex-col items-center justify-center w-full max-w-md h-32 border-2 border-dashed border-indigo-300 dark:border-indigo-800 rounded-lg cursor-pointer bg-indigo-50/50 dark:bg-indigo-950/20 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Click to upload Excel file</p>
              ...
              <div className="flex-1 space-y-2 text-sm text-slate-600 dark:text-slate-400">
                <p><strong className="text-slate-800 dark:text-slate-200">Required format:</strong></p>
                <ul className="list-disc pl-5 space-y-1">
                  <li>Column: <code>Our ASIN</code> (The ASIN you want to track)</li>
                  <li>Columns: <code>Competitor-1</code>, <code>Competitor-2</code>, etc.</li>
                </ul>
                <p className="text-xs mt-4 text-slate-500">Rows without "Our ASIN" will be safely ignored (e.g., Flipkart specific rows).</p>
              </div>
            </div>
            ...
            {inputs.length > 0 && (
              <div className="mb-4 flex justify-end">
                <button 
                  ...
                  disabled={isSaving} 
                  className="flex items-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 dark:text-indigo-300 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
                >
                  <Save className="w-4 h-4" /> {isSaving ? "Saving..." : "Save this list for later"}
                </button>
              </div>
            )}
            <LiveCompetitorDashboard initialInputs={inputs} />
          </div>
        </>
      )}
    </div>
