...
//           {tier === "premium" ? t('sellerPages.premiumPlan', 'Premium Â· â‚¹2,999/mo') : t('sellerPages.basicPlan', 'Basic Â· â‚¹1,999/mo')}
//         </p>
//       </div>
//       <button
...
//         <span className={`text-xs font-medium ${isDark ? 'text-red-400' : 'text-red-600'}`}>{t('sellerPages.analysisInterrupted', 'Analysis interrupted. Retry to continue â€” no data lost.')}</span>
//       )}
...
//                         <p className={`text-2xl font-black ${isDark ? 'text-slate-600' : 'text-slate-300'}`}>â€”</p>
...
//                           Recommended range: <span className={`font-black ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{fmt(reprice.rec_price_low, currency)} â€“ {fmt(reprice.rec_price_high, currency)}</span>
//                         </p>
...
//                             >
//                               {repriceStream.streaming ? <><RefreshCw className="w-3 h-3 animate-spin" /> Thinkingâ€¦</> : "âœ¦ Get AI advice"}
//                             </button>
//                           </div>
//                         </div>
...
//                     <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
//                       {[
//                         { label: "Your rating",        value: priceGap.your_rating ? `${priceGap.your_rating}â˜…` : "â€”",      color: isDark ? "text-amber-400" : "text-amber-500", tip: "Your product's average customer star rating." },
//                         { label: "Market avg rating",  value: `${priceGap.market_avg_rating}â˜…`,                              color: isDark ? "text-amber-300" : "text-amber-400", tip: "The average customer star rating across your competitors." },
...
//                         { label: "Your MRP discount",  value: priceGap.your_mrp_discount_pct ? `${priceGap.your_mrp_discount_pct}%` : "â€”", color: isDark ? "text-slate-300" : "text-slate-700", tip: "The percentage discount you are offering below your MRP." },
//                       ].map((s) => (
//                         <div key={s.label} className={`rounded-2xl p-4 border shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
//                           <div className={`text-xs mb-1 flex items-center gap-1 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>
//                             {s.label}
...
//                               {c.rating && <p className="text-[10px] text-amber-500 font-bold">{c.rating}â˜…</p>}
//                             </div>
//                           </div>
//                         ))}
//                       </div>
...
//                                         {fmt(d.old_price, currency)} â†’ {fmt(d.new_price, currency)}
//                                       </p>
...
//                                   className={`flex items-center gap-2 px-4 py-2 text-white text-xs rounded-xl font-medium transition-all ${isDark ? 'bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-600' : 'bg-blue-600 hover:bg-blue-500 disabled:bg-slate-200 disabled:text-slate-400'}`}
//                                 >
//                                   {alertStream.streaming ? <><RefreshCw className="w-3 h-3 animate-spin" /> Thinkingâ€¦</> : "âœ¦ Analyse movements"}
//                                 </button>
//                               </div>
//                             </div>
//                             <div className="px-5 pb-5 pt-3">
...
//                     {!isBasic
//                       ? "Get full price gap, repricing engine, confidence scores â€” Basic Â· â‚¹1,999/mo"
//                       : "Detect competitor price drops in real time, AI explanation â€” Premium Â· â‚¹2,999/mo"}
//                   </p>
