...
function Section({
  title, icon: Icon, children, defaultOpen = true, count, accent,
}: {
  title: string; icon: any; children: React.ReactNode;
  defaultOpen?: boolean; count?: number; accent?: string;
}) {
  ...
  return (
    <div className="mt-3 min-h-12 max-h-72 overflow-y-auto bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
      {stream.streaming && !stream.text && (
        <span className="text-slate-400 text-xs animate-pulse">Analysing...</span>
      )}
      ...
      <main className="flex-1 py-6 space-y-5">
        ...
        {!asin && (
          <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Select a product from My Products to run the price optimizer.</p>
            </div>
        ...
        {asin && loading && (
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <p className="text-slate-600 font-semibold">Loading product data…</p>
          </div>
        )}
        ...
        {asin && !loading && profile && (
          <>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                  ...
                  {profile.is_amazon_choice && <Badge className="text-[10px] bg-amber-50 text-amber-700 border-amber-200 px-1.5 py-0">AMAZON'S CHOICE</Badge>}
                ...
                {profile.star_rating && (
                  <p className="text-xs text-amber-500 font-bold mt-1">
                    {profile.star_rating}★
                    {profile.num_ratings && <span className="text-slate-400 font-normal ml-1">({profile.num_ratings.toLocaleString()} ratings)</span>}
                  </p>
                )}
              </div>
              ...
              <div className="shrink-0 text-right">
                <div className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-bold border-2 ${
                  profile.price_position === "Above market" ? "bg-purple-50 border-purple-300 text-purple-800"
                  : profile.price_position === "Below market" ? "bg-amber-50 border-amber-300 text-amber-800"
                ...
                <p className="text-xs text-slate-400 mt-1">{fmt(profile.your_price, currency)} vs {fmt(profile.market_avg, currency)} avg</p>
                <p className="text-xs text-slate-400">{pct(profile.pct_vs_avg)} vs market</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Your price",    value: fmt(profile.your_price, currency),  color: "text-slate-800" },
                { label: "Market avg",    value: fmt(profile.market_avg, currency),  color: "text-sky-600" },
                { label: "vs Market",     value: pct(profile.pct_vs_avg),            color: (profile.pct_vs_avg || 0) > 0 ? "text-purple-600" : "text-amber-600" },
                { label: "Action signal", value: (profile.recommended_action_teaser || "—").toUpperCase(), color: profile.recommended_action_teaser === "raise" ? "text-green-600" : profile.recommended_action_teaser === "lower" ? "text-red-600" : "text-slate-600" },
              ].map((s) => (
            ...
            {!isBasic && (
              <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
                <TierGate tier="basic" feature="Full Price Gap & Repricing Engine" />
              ...
              <>
                ...
                {activeTab === "reprice" && !tabLoading && reprice && (
                  <div className="space-y-4">
                    {/* Confidence + Rec */}
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                      <ConfidenceRing score={reprice.confidence_score} />
                      <div className="flex-1">
                        <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1">Recommended action</p>
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-base font-black border-2 ${
                          reprice.recommended_action === "raise" ? "bg-green-50 border-green-300 text-green-800"
                          : reprice.recommended_action === "lower" ? "bg-red-50 border-red-300 text-red-800"
                        ...
                        <p className="text-sm text-slate-600 mt-2">
                          Recommended range: <span className="font-black text-slate-800">{fmt(reprice.rec_price_low, currency)} – {fmt(reprice.rec_price_high, currency)}</span>
                        </p>
                        <p className="text-xs text-slate-400 mt-1">Currently: {fmt(reprice.your_price, currency)}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {[
                        ...
                        <div key={s.label} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
                          <div className="flex items-center gap-2 mb-1 text-slate-400">{s.icon}<span className="text-xs font-semibold">{s.label}</span></div>
                    ...
                    {reprice.alerts?.length > 0 && (
                      <div className="space-y-2">
                        {reprice.alerts.map((a, i) => <AlertBox key={i} type={a.type} message={a.message} />)}
                      </div>
                    )}

                    {/* AI rationale — premium */}
                    <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                      {!isPremium && <TierGate tier="premium" feature="AI Repricing Rationale" />}
                      <div className={!isPremium ? "blur-sm pointer-events-none" : ""}>
                        <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-slate-800 text-sm">AI repricing rationale</span>
                          </div>
                          <div className="flex gap-2">
                            {repriceStream.text && (
                              <button onClick={repriceStream.reset} className="text-xs text-slate-400 hover:text-slate-600">Clear</button>
                            )}
                            <button
                        ...
                        <div className="px-5 pb-5 pt-3">
                          {repriceStream.text || repriceStream.streaming || repriceStream.error
                            ? <StreamBox stream={repriceStream} />
                            : <div className="bg-slate-50 rounded-xl p-5 text-center border border-slate-100">
                                <p className="text-slate-400 text-xs">Click "Get AI advice" for a natural-language explanation of this recommendation</p>
                              </div>
                          }
                        </div>
                      </div>
                ...
                {activeTab === "gap" && !tabLoading && priceGap && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        { label: "Your price",    value: fmt(priceGap.your_price, priceGap.currency),   color: "text-slate-800" },
                        { label: "Market avg",    value: fmt(priceGap.market_avg, priceGap.currency),   color: "text-sky-600" },
                        { label: "Market low",    value: fmt(priceGap.market_min, priceGap.currency),   color: "text-green-600" },
                        { label: "Market high",   value: fmt(priceGap.market_max, priceGap.currency),   color: "text-purple-600" },
                      ].map((s) => (
                        <div key={s.label} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                          <p className="text-xs text-slate-400 mb-1">{s.label}</p>
                          <p className={`text-xl font-black ${s.color}`}>{s.value}</p>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        { label: "Your rating",        value: priceGap.your_rating ? `${priceGap.your_rating}★` : "—",      color: "text-amber-500" },
                        { label: "Market avg rating",  value: `${priceGap.market_avg_rating}★`,                              color: "text-amber-400" },
                        { label: "Rating gap",         value: pct(priceGap.rating_gap),                                      color: (priceGap.rating_gap || 0) >= 0 ? "text-green-600" : "text-red-600" },
                        { label: "Your MRP discount",  value: priceGap.your_mrp_discount_pct ? `${priceGap.your_mrp_discount_pct}%` : "—", color: "text-slate-700" },
                      ].map((s) => (
                        <div key={s.label} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                          <p className="text-xs text-slate-400 mb-1">{s.label}</p>
                          <p className={`text-xl font-black ${s.color}`}>{s.value}</p>
                    ...
                    {/* Price band chart */}
                    <Section title="Price band distribution" icon={BarChart2} defaultOpen={true} accent="bg-sky-50">
                      <p className="text-xs text-slate-400 mb-4">How many competitors sit in each price band. Your band is highlighted.</p>
                      <div className="space-y-3">
                        {priceGap.price_bands.map((b, i) => {
                          const max = Math.max(...priceGap.price_bands.map((x) => x.count), 1);
                          return (
                            <div key={i} className={`flex items-center gap-3 p-2.5 rounded-xl border ${b.your_price_in_band ? "border-blue-200 bg-blue-50" : "border-slate-100 bg-white"}`}>
                              <span className="text-xs text-slate-600 w-32 shrink-0 font-mono">
                                {sym(priceGap.currency)}{b.band}
                              </span>
                              <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full rounded-full transition-all duration-500"
                                  style={{ width: `${(b.count / max) * 100}%`, background: b.your_price_in_band ? "#3b82f6" : "#94a3b8" }}
                                />
                              </div>
                              <span className="text-xs text-slate-400 w-20 text-right shrink-0">{b.count} products</span>
                              {b.your_price_in_band && (
                                <span className="text-[10px] font-bold text-blue-600 bg-white px-2 py-0.5 rounded-full border border-blue-200">Your band</span>
                              )}
                            </div>
                          );
                        })}
                    ...
                    {/* Competitor table */}
                    <Section title="Competitors in this category" icon={Target} count={priceGap.competitors.length} defaultOpen={false} accent="bg-purple-50">
                      <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                        {priceGap.competitors.map((c, i) => (
                          <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${c.price_num === priceGap.your_price ? "bg-blue-50 border-blue-200" : "bg-slate-50 border-slate-100"}`}>
                            <span className="text-xs text-slate-400 w-4 shrink-0 font-mono">#{i + 1}</span>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-semibold text-slate-700 line-clamp-1">{c.product_title}</p>
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-sm font-black text-slate-800">{c.product_price}</p>
                ...
                {activeTab === "alerts" && (
                  <div className="relative overflow-hidden">
                    {!isPremium && (
                      <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
                        <TierGate tier="premium" feature="Competitor Price Movement Alerts" />
                        <div className="blur-sm pointer-events-none space-y-3">
                          {["Competitor A", "Competitor B", "Competitor C"].map((n, i) => (
                            <div key={i} className="flex items-center gap-3 p-3 bg-red-50 border border-red-100 rounded-xl">
                              <span className="text-xs font-mono font-bold text-slate-800 flex-1">{n}</span>
                              <span className="text-xs text-red-600 font-bold">-12.5%</span>
                              <span className="text-xs text-slate-400">Undercuts you</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {isPremium && !tabLoading && alertData && (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {[
                            { label: "Total competitors", value: String(alertData.total_competitors), color: "text-slate-800" },
                            { label: "Price movers",      value: String(alertData.price_movers),      color: "text-amber-600" },
                            { label: "Undercutting you",  value: String(alertData.undercuts_you),     color: alertData.undercuts_you > 0 ? "text-red-600" : "text-green-600" },
                          ].map((s) => (
                        ...
                        {alertData.deltas.length > 0 && (
                          <Section title="Price movements" icon={Bell} count={alertData.deltas.length} accent="bg-amber-50" defaultOpen={true}>
                            <div className="space-y-2">
                              {alertData.deltas.map((d, i) => (
                                <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${d.new_price < alertData.your_price ? "bg-red-50 border-red-200" : "bg-slate-50 border-slate-100"}`}>
                                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${d.direction === "down" ? "bg-red-100" : "bg-green-100"}`}>
                                    {d.direction === "down"
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-semibold text-slate-700">{d.seller_name}</p>
                                    <p className="text-[10px] text-slate-400">{d.updated_at?.split("T")[0]}</p>
                                  </div>
                                  <div className="text-right shrink-0">
                                    <p className="text-xs font-bold text-slate-800">
                                      {fmt(d.old_price, currency)} → {fmt(d.new_price, currency)}
                                    </p>
                                    <p className={`text-xs font-black ${d.change_pct < 0 ? "text-red-600" : "text-green-600"}`}>
                                      {pct(d.change_pct)}
                                    </p>
                                  </div>
                                  {d.new_price < alertData.your_price && (
                                    <span className="text-[10px] font-bold text-red-700 bg-red-100 px-2 py-0.5 rounded-full shrink-0">Undercuts you</span>
                                  )}
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
                            <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-slate-800 text-sm">AI alert analysis</span>
                              </div>
                              <div className="flex gap-2">
                                {alertStream.text && (
                                  <button onClick={alertStream.reset} className="text-xs text-slate-400 hover:text-slate-600">Clear</button>
                                )}
                                <button
                            ...
                            <div className="px-5 pb-5 pt-3">
                              {alertStream.text || alertStream.streaming || alertStream.error
                                ? <StreamBox stream={alertStream} />
                                : <p className="text-slate-400 text-xs bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
