...
export default function Page() {
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center select-none">
      ...
      <Link
        href="/dashboard"
        className="px-6 py-2.5 rounded-xl bg-[#0f2a43] text-white text-sm font-semibold hover:bg-[#1a3d5c] transition-colors"
      >
        Back to Dashboard
      </Link>
    </div>
...
//   "Storage Capacity":      { color: "text-blue-600",   colorDark: "text-blue-400",   bg: "bg-blue-50",   bgDark: "bg-blue-900/30",   border: "border-blue-200",   borderDark: "border-blue-800/50",   emoji: "ðŸ’¾" },
//   "Compatibility":         { color: "text-violet-600", colorDark: "text-violet-400", bg: "bg-violet-50", bgDark: "bg-violet-900/30", border: "border-violet-200", borderDark: "border-violet-800/50", emoji: "ðŸ”Œ" },
//   "Durability & Build":    { color: "text-emerald-600",colorDark: "text-emerald-400",bg: "bg-emerald-50",bgDark: "bg-emerald-900/30",border: "border-emerald-200",borderDark: "border-emerald-800/50",emoji: "ðŸ›¡ï¸" },
//   "Format & Standard":     { color: "text-amber-600",  colorDark: "text-amber-400",  bg: "bg-amber-50",  bgDark: "bg-amber-900/30",  border: "border-amber-200",  borderDark: "border-amber-800/50",  emoji: "ðŸ“‹" },
//   "Brand & Certification": { color: "text-sky-600",    colorDark: "text-sky-400",    bg: "bg-sky-50",    bgDark: "bg-sky-900/30",    border: "border-sky-200",    borderDark: "border-sky-800/50",    emoji: "âœ…" },
//   "Use Case":              { color: "text-pink-600",   colorDark: "text-pink-400",   bg: "bg-pink-50",   bgDark: "bg-pink-900/30",   border: "border-pink-200",   borderDark: "border-pink-800/50",   emoji: "ðŸŽ¯" },
...
//   if (sim >= 0.5)
//     return <span title="Semantically close â€” easy to add" className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${isDark ? 'text-emerald-400 bg-emerald-900/30 border-emerald-800/50' : 'text-emerald-600 bg-emerald-50 border-emerald-200'}`}>â‰ˆ Similar</span>;
//   if (sim >= 0.25)
...
//           {tier === "premium" ? "Premium Â· â‚¹2,999/mo" : "Basic Â· â‚¹1,999/mo"}
//         </p>
//       </div>
//       <button
...
//         {item.in_yours
//           ? <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isDark ? 'text-sky-400 bg-sky-900/30' : 'text-sky-600 bg-sky-50'}`}>âœ“ Yours</span>
...
//           <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${isDark ? 'text-emerald-400 bg-emerald-900/30 border-emerald-800/50' : 'text-emerald-600 bg-emerald-50 border-emerald-200'}`}>â‰ˆ Similar</span>
//           <span className={`text-[9px] self-center ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>easy to rephrase into title</span>
//           <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${isDark ? 'text-red-400 bg-red-900/30 border-red-800/50' : 'text-red-600 bg-red-50 border-red-200'}`}>âœ¦ New concept</span>
...
//             Showing top 30 of {filtered.length} â€” narrow filter to see more
//           </p>
//         )}
//       </div>
//     </div>
...
//                 <p className={`font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Analysing keyword gapsâ€¦</p>
//                 <p className={`text-xs animate-pulse mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>We are analyzing the data. This may take 1â€“2 minutes.</p>
//               </div>
//             </div>
//           )}
// 
...
//                       {[
//                         { label: "Gap Keywords", value: data.gap_count_teaser ?? "â€”", color: "text-red-500" },
//                         { label: "Coverage Score", value: "â€”/100", color: "text-amber-500" },
//                         { label: "Competitors Scanned", value: "â€”", color: isDark ? "text-sky-400" : "text-sky-600" },
//                       ].map((s) => (
...
//                   {[
//                     { label: "Gap Keywords",    value: String(data.gap_count_teaser ?? "â€”"), sub: "not in your title", color: isDark ? "text-red-400" : "text-red-500", helpText: "These are important words that your competitors are using, but you are not. Adding them can help increase your ranking." },
...
//                     { label: "Coverage Score",  value: `${data.coverage_score ?? "â€”"}/100`,  sub: "vs top 40 competitor KWs", color: isDark ? "text-sky-400" : "text-sky-600", helpText: "This score shows how strong your title is compared to the top sellers. 100/100 is the best possible score." },
//                   ].map((s) => (
...
//                     : <p className={`text-sm text-center py-6 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No gap keywords â€” excellent coverage!</p>
//                   }
//                 </Section>
//               )}
// 
...
//                     <p className={`text-xs mb-3 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>Keywords you share with competitors â€” good coverage here.</p>
//                     <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto">
//                       {sharedKeywords.map((s: any, i: number) => (
//                         <div key={i} className="flex items-center gap-1">
//                           <KwPill kw={s.keyword} variant="shared" isDark={isDark} />
//                           {s.comp_freq > 1 && <span className={`text-[9px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Ã—{s.comp_freq}</span>}
//                         </div>
//                       ))}
...
//                             <p className="text-xs font-black text-amber-500">{c.star_rating}â˜…</p>
...
//                     <p className={`text-xs mb-4 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>
//                       Keywords your customers actually use in reviews â€” often missed in titles but highly converting.
//                     </p>
...
//                       AI-ranked by opportunity value (1â€“10). Includes placement advice â€” where to add each keyword.
//                     </p>
...
//                       ))
//                     ) : isPremium ? (
//                       <p className={`text-sm text-center py-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>AI scoring not available â€” no gap keywords found.</p>
//                     ) : (
...
//                       </>
//                     ) : isPremium ? (
//                       <p className={`text-sm text-center py-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Rewrite not available â€” insufficient gap data.</p>
//                     ) : (
//                       <div>
//                         <div className="mb-3">
//                           <p className={`text-[10px] font-bold uppercase tracking-wide mb-1.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Current Title</p>
...
//                           "Keep your differentiators â€” they are unique to your listing",
//                         ].map((step, i) => (
//                           <div key={i} className={`flex items-start gap-3 p-3 border rounded-xl ${isDark ? 'bg-emerald-900/10 border-emerald-900/30' : 'bg-emerald-50 border-emerald-100'}`}>
//                             <div className="w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5">
//                               <span className="text-xs font-black text-white">{i + 1}</span>
...
//                       {!isBasic
//                         ? "Get gap keywords, semantic clusters, coverage score & heatmap â€” Basic Â· â‚¹1,999/mo"
//                         : "Get AI opportunity scores, listing rewrite, placement advice & action plan â€” Premium Â· â‚¹2,999/mo"}
//                     </p>
