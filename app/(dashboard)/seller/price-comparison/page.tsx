...
//               <p className={`font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Finding similar products & analysing pricesâ€¦</p>
...
//                   { label: "Discount", value: data.discount_pct != null ? `${data.discount_pct}%` : "â€”", sub: "off MRP", tip: "How much percentage discount you are offering below the MRP. A higher discount attracts more buyers." },
//                   { label: "No. of Offers", value: data.num_offers != null ? String(data.num_offers) : "â€”", sub: "competing sellers", tip: "How many different sellers are offering this same product. More sellers = more competition for the Buy Box." },
//                 ].map((s: any) => (
...
//                   { label: "Market Average", value: fmt(data.market_avg, currency), sub: `${data.competitor_count ?? "â€”"} similar products`, tip: "The average selling price of similar products in your category. Use this as a benchmark for your pricing strategy." },
//                   { label: "Market Range", value: `${fmtShort(data.market_min, currency)} â€“ ${fmtShort(data.market_max, currency)}`, sub: "min â€“ max", wide: false, tip: "The price range across all competitors â€” from the cheapest to the most expensive product in this category." },
//                   { label: "Your Position", value: null, position: data.price_position, sub: data.price_percentile != null ? `top ${data.price_percentile}% of market` : null, tip: "Where your price sits relative to the market â€” 'Cheap', 'Competitive', 'Premium', or 'Expensive'." },
//                 ].map((s: any) => (
...
//                     <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>Based on {data.competitor_count ?? "â€”"} similar products</p>
//                   </div>
...
//                 <p className={`text-xs mb-4 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>Green = less competition Â· Red = crowded Â· Blue = your price band</p>
//                 <div className={`space-y-3 ${!isBasic ? "blur-sm pointer-events-none" : ""}`}>
//                   {(data.price_bands || Array(5).fill({ label: "â€”", count: 0, density: "Low", your_price_in_band: false })).map((b: any, i: number) => {
...
//                           {b.your_price_in_band ? "â† You" : b.density}
//                         </span>
//                       </div>
//                     );
//                   })}
...
//                       {data.ai_pricing_tip || "Analysing your price vs market trendsâ€¦"}
//                     </p>
...
//                       {!isBasic
//                         ? "Get smart competitor matching, market benchmarks & AI pricing tips â€” Basic Â· â‚¹1,999/mo"
//                         : "Get AI pricing recommendations and cross-product comparisons â€” Premium Â· â‚¹2,999/mo"}
//                     </p>
