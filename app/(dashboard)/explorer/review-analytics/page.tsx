  ...
  return (
    <div className="absolute inset-0 bg-background backdrop-blur-none rounded-2xl flex flex-col items-center justify-center z-10 gap-3">
      ...
      <div className="text-center px-4">
        ...
        <p className="text-xs text-slate-400 mt-0.5">{tier === "premium" ? t("reviewAnalytics.premiumPrice", "Premium · ₹2,999/mo") : t("reviewAnalytics.basicPrice", "Basic · ₹1,999/mo")}</p>
      </div>
      <button onClick={() => router.push("/subscription")}
        ...
        <Crown className="w-3 h-3" /> {t("reviewAnalytics.upgrade", "Upgrade")}
      </button>
    </div>
  );
}
...
function HealthScoreRing({ score }: { score: number }) {
  ...
  const label = score >= 75 ? t("reviewAnalytics.strong", "Strong") : score >= 50 ? t("reviewAnalytics.moderate", "Moderate") : t("reviewAnalytics.needsWork", "Needs Work");
  return (
    ...
    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {review.has_response && (
            <span className="text-[10px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">{t("reviewAnalytics.replied", "Replied")}</span>
          )}
        </div>
      </div>
...
function SimilarityPill({ score }: { score: number }) {
  ...
  return (
    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${cls}`}>{pct}% {t("reviewAnalytics.match", "match")}</span>
  );
}
...
const ChartTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-2.5 shadow-lg text-xs">
      <p className="font-semibold text-slate-600">{label} {t("reviewAnalytics.star", "star")}</p>
      <p className="text-sky-600 font-bold">{payload[0]?.value} {t("reviewAnalytics.reviews", "reviews")}</p>
    </div>
  );
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-sky-100 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-sky-900">{t("reviewAnalytics.reviewComparison", "Review Comparison")}</h2>
            <p className="text-xs text-slate-500 hidden sm:block">{t("reviewAnalytics.subtitle", "Analyse your reviews and benchmark against similar products")}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {!asin && (
        <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
          ...
          <div>
            <p className="text-lg font-bold text-slate-700">{t("reviewAnalytics.noProduct", "No product selected")}</p>
            <p className="text-sm text-slate-400 mt-1">{t("reviewAnalytics.clickProduct", "Click any product from My Products to compare reviews.")}</p>
          </div>
          <button onClick={() => router.push("/seller/my-products")} className="px-5 py-2 bg-sky-600 text-white rounded-full text-sm font-semibold hover:bg-sky-700 transition-colors">
            {t("reviewAnalytics.goToMyProducts", "Go to My Products")}
          </button>
        </div>
      )}
      ...
      {asin && loading && (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
          ...
          <p className="text-slate-500 font-medium">{t("reviewAnalytics.analysing", "Analysing reviews & finding similar competitors…")}</p>
        </div>
      )}
      ...
      {asin && !loading && data && (
        <>
          {/* ── Product Card ──────────────────────────────────────────────── */}
          <div className="bg-background rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="text-right flex-shrink-0 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{data.total_ratings?.toLocaleString() || "—"} {t("reviewAnalytics.ratings", "ratings")}</p>
                {data.seller_rating && <p className="text-xs text-slate-500 mt-0.5">{t("reviewAnalytics.sellerRating", "Seller Rating")}: ★ {data.seller_rating}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h3 className="font-bold text-slate-800 text-sm mb-4">{t("reviewAnalytics.ratingOverview", "Rating Overview")}</h3>
              <div className="flex items-center gap-5 mb-5">
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{data.total_ratings?.toLocaleString()} {t("reviewAnalytics.total", "total")}</p>
                </div>
              ...
              {data.seller_rating && (
                <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-medium">{t("reviewAnalytics.sellerRating", "Seller Rating")}</span>
                  <span className="font-bold text-slate-800 flex items-center gap-1">
                    <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                    {data.seller_rating} ({data.seller_ratings_total?.toLocaleString()} {t("reviewAnalytics.ratings", "ratings")})
                  </span>
                </div>
              )}
            </div>
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h3 className="font-bold text-slate-800 text-sm mb-4">{t("reviewAnalytics.ratingDistribution", "Rating Distribution")}</h3>
          ...
          <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
            ...
            <div className={!isBasic ? "blur-sm pointer-events-none" : ""}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-slate-800 text-sm">{t("reviewAnalytics.recentReviews", "Recent Customer Reviews")}</h3>
                {data.response_rate_label && (
                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${(data.response_rate_pct || 0) > 50 ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-amber-50 text-amber-600 border-amber-200"}`}>
                    {data.response_rate_label} {t("reviewAnalytics.responded", "responded")}
                  </span>
                )}
              </div>
              {(data.recent_reviews || []).length > 0
                ? <div className="space-y-3">{(data.recent_reviews || []).map((r: any, i: number) => <ReviewCard key={i} review={r} />)}</div>
                : <p className="text-sm text-slate-400 text-center py-8">{t("reviewAnalytics.noRecentReviews", "No recent reviews tracked.")}</p>
              }
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
              ...
              <div className={!isBasic ? "blur-sm pointer-events-none" : ""}>
                <h3 className="font-bold text-slate-800 text-sm mb-3">{t("reviewAnalytics.sellerResponseRate", "Seller Response Rate")}</h3>
                <div className="flex items-end gap-3">
                  <p className="text-4xl font-black text-sky-600">{data.response_rate_pct != null ? `${data.response_rate_pct}%` : "—"}</p>
                  <p className="text-sm text-slate-500 pb-1">{data.response_rate_label || t("reviewAnalytics.ofReviews", "of reviews")}</p>
                </div>
            ...
            <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
              ...
              <div className={!isPremium ? "blur-sm pointer-events-none" : ""}>
                <h3 className="font-bold text-slate-800 text-sm mb-3">{t("reviewAnalytics.yourPortfolioAvg", "Your Portfolio Average")}</h3>
                <div className="flex items-end gap-3">
                ...
                <p className="text-xs text-slate-500 mt-2">{t("reviewAnalytics.averageAcross", "Average across")} {data.seller_product_count || "—"} {t("reviewAnalytics.trackedProducts", "tracked products")}</p>
                {data.star_rating && data.avg_seller_portfolio_rating && (
                  <div className="mt-3 bg-slate-50 rounded-xl p-2.5 text-xs flex items-center justify-between">
                    <span className="text-slate-500">{t("reviewAnalytics.thisVsPortfolio", "This product vs portfolio")}</span>
                    <span className={`font-bold ${data.star_rating >= data.avg_seller_portfolio_rating ? "text-emerald-600" : "text-red-500"}`}>
                      {data.star_rating >= data.avg_seller_portfolio_rating ? `↑ ${t("reviewAnalytics.aboveAvg", "Above avg")}` : `↓ ${t("reviewAnalytics.belowAvg", "Below avg")}`}
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
            ...
            <div className={!isPremium ? "blur-sm pointer-events-none" : ""}>
              <h3 className="font-bold text-slate-800 text-sm mb-4">{t("reviewAnalytics.reviewSentiment", "Review Sentiment Breakdown")}</h3>
              <div className="space-y-3">
                <SentimentBar label={`${t("reviewAnalytics.positive", "Positive")} (4–5 ★)`} pct={data.sentiment_breakdown?.positive ?? 80} icon={ThumbsUp} color="#10b981" />
                <SentimentBar label={`${t("reviewAnalytics.neutral", "Neutral")} (3 ★)`} pct={data.sentiment_breakdown?.neutral ?? 12} icon={Minus} color="#f59e0b" />
                <SentimentBar label={`${t("reviewAnalytics.negative", "Negative")} (1–2 ★)`} pct={data.sentiment_breakdown?.negative ?? 8} icon={ThumbsDown} color="#ef4444" />
              </div>
            </div>
          </div>
          ...
          <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
            ...
            <div className={!isPremium ? "blur-sm pointer-events-none" : ""}>
              <h3 className="font-bold text-slate-800 text-sm mb-1 flex items-center gap-2">
                <Users className="w-4 h-4 text-sky-500" /> {t("reviewAnalytics.similarCompetitors", "Most Similar Competitors by Reviews")}
              </h3>
              <p className="text-xs text-slate-400 mb-4">{t("reviewAnalytics.matchedBy", "Matched by product similarity, not just category")}</p>
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 p-3 bg-sky-50 rounded-xl border border-sky-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 font-mono">{data.asin} · {t("reviewAnalytics.you", "YOU")}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-black text-amber-500">{data.star_rating?.toFixed(1)}</p>
                    <p className="text-[10px] text-slate-400">{data.total_ratings?.toLocaleString()} {t("reviewAnalytics.ratings", "ratings")}</p>
                  </div>
                </div>
                {(data.competitor_reviews || []).map((c: any, i: number) => (
                  <div 
                    ...
                    onClick={() => router.push(`/product/${encodeURIComponent(c.title)}?from=reviews&source=amazon`)}
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-[10px] text-slate-400">{c.num_ratings?.toLocaleString()} {t("reviewAnalytics.ratings", "ratings")}</p>
                    </div>
                  </div>
                ))}
                {(data.competitor_reviews || []).length === 0 && (
                  <div className="p-4 text-center rounded-xl border border-dashed border-slate-200 text-slate-400">
                    <p className="text-xs">{t("reviewAnalytics.noDirectCompetitors", "No direct competitors found matching your specific product keywords.")}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden">
              ...
              <div className={!isPremium ? "blur-sm pointer-events-none" : ""}>
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-bold text-slate-800 text-sm mb-1">{t("reviewAnalytics.aiDrafted", "AI-Drafted Review Response")}</p>
                    <p className="text-sm text-slate-600 leading-relaxed">{data.ai_response_suggestion || t("reviewAnalytics.generating", "Generating personalized response…")}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {isPremium && data.review_velocity_insight && (
            <div className="bg-gradient-to-r from-sky-50 to-blue-50 rounded-2xl border border-sky-100 p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-sky-700 mb-0.5">{t("reviewAnalytics.reviewVelocity", "Review Velocity Insight")}</p>
          ...
          {!isPremium && (
            <div className="bg-gradient-to-r from-blue-600 to-cyan-500 rounded-2xl p-5 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <p className="font-bold text-base">{t("reviewAnalytics.unlockFull", "Unlock Full Review Intelligence")}</p>
              ...
              <button onClick={() => router.push("/subscription")} className="flex items-center gap-2 px-5 py-2.5 bg-white text-blue-600 rounded-full font-bold text-sm shadow hover:shadow-md hover:scale-105 transition-all flex-shrink-0">
                <Crown className="w-4 h-4" /> {t("reviewAnalytics.upgradeNow", "Upgrade Now")}
              </button>
            </div>
          )}
        </>
