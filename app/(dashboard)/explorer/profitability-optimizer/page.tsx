      ...
      {activeMode === "analyze" && (
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex gap-2">
              {analyzeStream.text && (
                <button onClick={analyzeStream.reset} className="text-xs text-slate-400 hover:text-slate-600 transition-colors" data-track-id="ai_run_analysis_clear_btn">
                  Clear
                </button>
              )}
              <button
    ...
    <div className="space-y-6">
      ...
      {saveModal && (
        <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-900 mb-1">Save product</h3>
            <p className="text-xs text-slate-400 mb-4">Saved to your account · accessible on any device</p>
            <input
              type="text"
              value={saveName}
              onChange={(e) => setSaveName(e.target.value)}
            ...
            {calcResult && (
              <div className="bg-slate-50 rounded-xl p-3 mb-4 border border-slate-100 text-xs text-slate-500 space-y-1">
                ...
                <div className="flex justify-between"><span>Monthly profit</span><span className="font-semibold text-slate-700">{inr(calcResult.monthly_profit)}</span></div>
              </div>
            )}
            <div className="flex gap-3">
