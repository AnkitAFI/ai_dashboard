...
//                   <Lock className="w-3.5 h-3.5" /> Unlock Full Access
//                 </button>
//               )}
//             </CardContent>
//           </Card>
...
//                             onClick={() => setUpgrade({ open: true, feature: "Cost waterfall" })}>Unlock Full Access</button>
//                         </div>
//                       </div>
//                     )}
//                   </CardContent>
  ...
  const start = useCallback(async (url: string, body: object) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const codeStr =
          code !== null && typeof code === "object"
            ? (((code as Record<string, unknown>).error as string) ??
              "request_failed")
            : "request_failed";
        setError(codeStr);
        setStreaming(false);
...
function AlertBox({ type, message }: { type: string; message: string }) {
  const styles: Record<string, string> = {
    danger:
      "bg-red-50 dark:bg-red-950/20 border-red-400 dark:border-red-900/50 text-red-800 dark:text-red-400",
    warn: "bg-amber-50 dark:bg-amber-950/20 border-amber-400 dark:border-amber-900/50 text-amber-800 dark:text-amber-400",
    success:
      "bg-emerald-50 dark:bg-emerald-950/20 border-emerald-400 dark:border-emerald-900/50 text-emerald-800 dark:text-emerald-450",
  };
  const icons: Record<string, JSX.Element> = {
    danger: <XCircle className="w-4 h-4 shrink-0 mt-0.5" />,
    warn: <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />,
  ...
  return (
    <div
      className={`flex items-start gap-2 p-3 rounded-xl border-l-4 text-sm ${styles[type] ?? styles.warn}`}
    >
      {icons[type] ?? icons.warn}
      <span>{String(message ?? "")}</span>
    </div>
...
function SliderRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 py-1.5 ${locked ? "opacity-40 pointer-events-none" : ""}`}
    >
...
function AIPanel({
  ...
  useEffect(() => {
    fetch(`${API}/profitability/ai/status`)
      .then((r) => r.json())
      .then(setAiStatus)
      .catch(() => {});
  }, []);
  ...
  }, [chatStream.text, chatHistory]);

  useEffect(() => {
    if (!chatStream.streaming && chatStream.text && pendingChat) {
      setChatHistory((h) => [
        ...h,
        { role: "assistant", content: chatStream.text },
      ]);
      setPendingChat(null);
    }
  }, [chatStream.streaming]);
  ...
  const runAnalyze = () => {
    if (!isPremium) {
      onUpgrade("AI full analysis");
      return;
    }
    analyzeStream.reset();
    analyzeStream.start(`${API}/profitability/ai/analyze`, {
  ...
  const runChat = async () => {
    ...
    await chatStream.start(`${API}/profitability/ai/chat`, {
      question: q,
      calc_context: { ...calcResult, ...inputs },
      history: chatHistory,
      user_id: userId,
    });
  };
  ...
  const runScenario = () => {
    if (!isPremium) {
      onUpgrade("Scenario AI advice");
      return;
    }
    scenarioStream.reset();
    scenarioStream.start(`${API}/profitability/ai/scenario-advice`, {
      scenarios: scenarios ?? [],
      base_inputs: inputs,
      user_id: userId,
    });
  ...
  const runHealth = () => {
    if (!isPremium) {
      onUpgrade("Health action plan");
      return;
    }
    healthStream.reset();
    healthStream.start(`${API}/profitability/ai/health-advice`, {
  ...
  function StreamBox({
    ...
    stream: ReturnType<typeof useOllamaStream>;
  }) {
    return (
      <div className="mt-3 min-h-16 max-h-80 overflow-y-auto bg-white dark:bg-slate-800/60 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        {stream.error === "upgrade_required" && (
          <span className="text-amber-600 dark:text-amber-400 text-xs font-medium">
            Available on Premium plan. Upgrade to unlock.
          </span>
        )}
        {stream.error === "ollama_offline" && (
        ...
        {stream.error === "stream_interrupted" && (
          <span className="text-red-600 dark:text-red-400 text-xs font-medium">
            Analysis interrupted. Retry to continue — no data lost.
          </span>
        )}
        {stream.error &&
          ![
  ...
  const QUICK_Q = [
    ...
    "Should I raise price by 10%?",
    "What's eating my margin most?",
    "How do I hit 25% margin?",
  ];
          ...
          <div className="flex items-start justify-between mb-2">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                4 recommendations from your exact numbers
              </p>
            </div>
            <div className="flex gap-2">
              {analyzeStream.text && (
          ...
          analyzeStream.error ? (
            ...
            <div className="mt-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 text-center border border-slate-100 dark:border-slate-700">
              <p className="text-slate-400 dark:text-slate-500 text-xs">
                Click "Run analysis" for AI recommendations
              </p>
            </div>
          )}
        </div>
            ...
            {chatHistory.length === 0 && (
              <div className="py-4 text-center">
                <p className="text-slate-400 dark:text-slate-500 text-xs mb-3">
                  Ask anything about your product&apos;s numbers
                </p>
                <div className="flex flex-wrap gap-2 justify-center">
                  {QUICK_Q.map((q) => (
                    <button
              ...
              <div className="flex justify-start">
                <div className="max-w-[85%] px-3.5 py-2.5 text-xs bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-2xl rounded-bl-sm border border-slate-200 dark:border-slate-700 leading-relaxed">
                  {chatStream.text ? (
                    ...
                    <span className="animate-pulse text-slate-400 dark:text-slate-500">
                      ▌
                    </span>
                  )}
                </div>
        ...
        <div className="bg-white dark:bg-slate-900/60 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex items-start justify-between mb-2">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                AI compares your 4 scenarios and recommends one
              </p>
            </div>
            <button
              onClick={runScenario}
          ...
          {(scenarioStream.text ||
            scenarioStream.streaming ||
            scenarioStream.error) && <StreamBox stream={scenarioStream} />}
        </div>
      )}
      ...
      {activeMode === "health" && (
        <div className="bg-white dark:bg-slate-900/60 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex items-start justify-between mb-2">
            <div>
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
                5-step health improvement plan
              </p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                AI turns your health score into a prioritized action plan
              </p>
            </div>
            <button
              onClick={runHealth}
          ...
          {(healthStream.text ||
            ...
            healthStream.error) && <StreamBox stream={healthStream} />}
        </div>
      )}
    </div>
  );
...
export default function ProfitabilityOptimizer() {
  ...
  const [inputs, setInputs] = useState<Inputs>({
    ...
    shipping_to_fba: 180,
    fba_fee: 200,
    ad_spend_per_unit: 120,
    monthly_units: 300,
    return_rate_pct: 5,
    storage_fee_per_unit: 15,
    referral_fee_pct: 9,
  ...
  const [activeTab, setActiveTab] = useState<
    "calc" | "scenario" | "market" | "health" | "ai"
  >("calc");
  const [calcResult, setCalcResult] = useState<CalcResult | null>(null);
  const [scenarios, setScenarios] = useState<unknown[] | null>(null);
  const [sensitivity, setSensitivity] = useState<Record<string, unknown>[]>([]);
  const [marketIntel, setMarketIntel] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const [nicheKeyword, setNicheKeyword] = useState("");
  const [fetchingNiche, setFetchingNiche] = useState(false);
  const [nicheDataSource, setNicheDataSource] = useState<
    "db" | "ai_estimate" | null
  >(null);
  ...
  const fetchCategories = async (marketplace: string) => {
    try {
      const res = await axios.get(`${API}/profitability/categories`, {
        params: { marketplace },
      });
      const cats: string[] = res.data.categories ?? [];
      setCategories(cats);
    ...
    try {
      ...
      if (res.data?.success && res.data?.data) {
        ...
        const productCount: number =
          pricing.product_count || item.similar_products?.length || 0;
        ...
        const isElectronics =
          /electronic|laptop|phone|mobile|gadget|camera|tablet|audio/.test(cat);
        const isApparel = /cloth|fashion|apparel|wear|shirt|dress|shoe/.test(
          cat,
        );

        const cogsPct = isElectronics ? 0.35 : isApparel ? 0.25 : 0.3;
        const fbaPct = isElectronics ? 0.1 : 0.08;
        const shippingPct = 0.03;
        const adsPct = isElectronics ? 0.08 : 0.1;

        const calculatedCost = Math.max(
          50,
          Math.round(recommendedPrice * cogsPct),
        );
        const calculatedFba = Math.max(
          30,
          Math.round(recommendedPrice * fbaPct),
        );
        const calculatedShipping = Math.max(
          20,
          Math.round(recommendedPrice * shippingPct),
        );
        const calculatedAdSpend = Math.max(
          20,
          Math.round(recommendedPrice * adsPct),
        );
        ...
        toast(
          "No results",
          res.data?.message ||
            "No competitor products found. Try a different keyword.",
          "error",
        );
      }
    } catch (e) {
  ...
  const runCalculate = useCallback(async () => {
    ...
    setCalcLoading(true);
    try {
      const res = await axios.post(`${API}/profitability/calculate`, {
        ...inputs,
      });
      setCalcResult(res.data as CalcResult);
      setTier(String(res.data.tier ?? "free"));
  ...
  const fetchScenarios = async () => {
    ...
    setTabLoading(true);
    try {
      const res = await axios.post(`${API}/profitability/scenarios`, {
        ...inputs,
      });
      setScenarios(res.data.scenarios);
      setSensitivity(res.data.sensitivity);
  ...
  const fetchHealth = async () => {
    ...
    setTabLoading(true);
    try {
      const res = await axios.post(`${API}/profitability/health`, {
        ...inputs,
      });
      setHealthData(res.data as Record<string, unknown>);
    } catch (e: unknown) {
  ...
  const inp = (key: keyof Inputs, value: number | string) =>
  ...
  const handleSave = () => {
    ...
    if (savedProducts.length >= saveLimit) {
      toast(
        "Limit reached",
        `Upgrade to save more than ${saveLimit} products.`,
        "error",
      );
      return;
    }
  ...
  const waterfallData = useMemo(() => {
    ...
    return [
      {
        name: "Product cost",
        value: Number(bd.product_cost || 0),
        color: WATERFALL_COLORS.product_cost,
      },
      {
        name: "Shipping → FBA",
        value: Number(bd.shipping_to_fba || 0),
        color: WATERFALL_COLORS.shipping_to_fba,
      },
      {
        name: "FBA fee",
        value: Number(bd.fba_fee || 0),
        color: WATERFALL_COLORS.fba_fee,
      },
      {
        name: "Referral fee",
        value: Number(bd.referral_fee || 0),
        color: WATERFALL_COLORS.referral_fee,
      },
      {
        name: "Ad spend",
        value: Number(bd.ad_spend || 0),
        color: WATERFALL_COLORS.ad_spend,
      },
      {
        name: "Storage",
        value: Number(bd.storage_fee || 0),
        color: WATERFALL_COLORS.storage_fee,
      },
      {
        name: "Returns",
        value: Number(bd.return_cost || 0),
        color: WATERFALL_COLORS.return_cost,
      },
    ].filter((d) => d.value > 0);
  }, [calcResult]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/80 p-4 rounded-2xl border border-sky-100 dark:border-slate-700 shadow-sm">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-sky-900 dark:text-sky-300">
            Profitability Optimizer
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-0.5">
            Real margins · Live market data
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Badge
            className={`text-xs font-semibold ${tier === "premium" ? "bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300" : tier === "basic" ? "bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-300" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}
          >
            {tier.toUpperCase()}
          </Badge>
          {calcResult && (
                ...
                {[
                  ...
                  >
                    ...
                    {plan.feats.map((f) => (
                      <p
                        key={f}
                        className="text-slate-500 dark:text-slate-400 flex items-center gap-1 mb-0.5"
                      >
        ...
        <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && !savingProduct && confirmSave()
              }
              autoFocus
            />
            {calcResult && (
              <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3 mb-4 border border-slate-100 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-400 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-700 dark:text-slate-200">
                    {inr(calcResult.profit_per_unit)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Net margin</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-200">
                    {pct(calcResult.net_margin_pct)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Monthly profit</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-200">
                    {inr(calcResult.monthly_profit)}
                  </span>
                </div>
              </div>
            )}
      ...
      <div className="fixed bottom-4 right-4 z-[100] space-y-2 max-w-sm">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`flex items-start gap-3 p-4 rounded-xl shadow-lg border-2 ${t.variant === "success" ? "bg-green-50 dark:bg-green-950/60 border-green-300 dark:border-green-800" : "bg-red-50 dark:bg-red-950/60 border-red-300 dark:border-red-800"}`}
          >
            {t.variant === "success" ? (
        ...
        <CardContent className="p-0">
          <div className="flex border-b border-slate-200 dark:border-slate-700 overflow-x-auto">
            {(
              ...
              const locked =
                (tab.min === "premium" && !isPremium) ||
                (tab.min === "basic" && !isBasicPlus);
              return (
                <button
                  key={tab.id}
                  onClick={() =>
                    locked
                      ? setUpgrade({ open: true, feature: tab.label })
                      : handleTab(tab.id as typeof activeTab)
                  }
            ...
            <CardContent className="space-y-0.5 pt-0">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleNicheAutofill()
                      }
                    />
                  </div>
                  <Button
                  ...
                  >
                    {nicheDataSource === "db" ? (
                      <>
                        ...
                        Live DB data · {nicheProductCount} competitor
                        {nicheProductCount !== 1 ? "s" : ""}
                        {nicheConfidence
              ...
              <div className="mb-3">
                ...
                <select
                  ...
                  onChange={(e) => inp("marketplace", e.target.value)}
              ...
              <SliderRow
                ...
                max={Math.max(
                  10000,
                  Math.ceil((inputs.selling_price * 1.5) / 5000) * 5000,
                )}
                step={50}
                format={inr}
                onChange={(v) => inp("selling_price", v)}
              />
              <SliderRow
                label="Product cost"
                ...
                max={Math.max(
                  5000,
                  Math.ceil((inputs.product_cost * 1.5) / 1000) * 1000,
                )}
                step={25}
                format={inr}
                onChange={(v) => inp("product_cost", v)}
              />
              <SliderRow
                label="Shipping to FBA"
                value={inputs.shipping_to_fba}
                min={0}
                max={Math.max(
                  800,
                  Math.ceil((inputs.shipping_to_fba * 2) / 100) * 100,
                )}
                step={10}
                format={inr}
                onChange={(v) => inp("shipping_to_fba", v)}
              />
              <SliderRow
                label="FBA fee"
                value={inputs.fba_fee}
                min={0}
                max={Math.max(600, Math.ceil((inputs.fba_fee * 2) / 100) * 100)}
                step={10}
                format={inr}
                onChange={(v) => inp("fba_fee", v)}
              />
              <SliderRow
                label="Ad spend / unit"
                value={inputs.ad_spend_per_unit}
                min={0}
                max={Math.max(
                  800,
                  Math.ceil((inputs.ad_spend_per_unit * 2) / 100) * 100,
                )}
                step={5}
                format={inr}
                onChange={(v) => inp("ad_spend_per_unit", v)}
              />
              <SliderRow
                label="Monthly units"
                value={inputs.monthly_units}
                min={10}
                max={Math.max(
                  5000,
                  Math.ceil((inputs.monthly_units * 1.5) / 500) * 500,
                )}
                step={10}
                format={(v) => `${v}`}
                onChange={(v) => inp("monthly_units", v)}
              />
              <SliderRow
                label={`Referral (${Number(inputs.referral_fee_pct).toFixed(0)}%)`}
                value={inputs.referral_fee_pct}
                min={1}
                max={25}
                step={0.5}
                format={pct}
                onChange={(v) => inp("referral_fee_pct", v)}
              />
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {!isBasicPlus && (
                  <span
                    onClick={() =>
                      setUpgrade({ open: true, feature: "Advanced inputs" })
                    }
                    className="text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium cursor-pointer hover:bg-amber-200 dark:hover:bg-amber-800/40 transition-colors"
                  >
                    Basic+
                  </span>
                )}
              ...
              <SliderRow
                label="Return rate"
                value={inputs.return_rate_pct}
                min={0}
                max={40}
                step={1}
                format={pct}
                locked={!isBasicPlus}
                onChange={(v) => inp("return_rate_pct", v)}
              />
              <SliderRow
                label="Storage / unit"
                value={inputs.storage_fee_per_unit}
                min={0}
                max={150}
                step={2}
                format={inr}
                locked={!isBasicPlus}
                onChange={(v) => inp("storage_fee_per_unit", v)}
              />
              {!isBasicPlus && (
                <button
                  onClick={() =>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {(
                    [
                      {
                        label: "Profit / unit",
                        val: inr(Number(calcResult.profit_per_unit)),
                        cls:
                          Number(calcResult.profit_per_unit) > 0
                            ? Number(calcResult.net_margin_pct) > 20
                      ...
                      {
                        label: "Net margin",
                        val: pct(Number(calcResult.net_margin_pct)),
                        cls:
                          Number(calcResult.net_margin_pct) > 20
                            ? "text-green-600 dark:text-green-400"
                            : Number(calcResult.net_margin_pct) > 10
                              ? "text-amber-600 dark:text-amber-400"
                              : "text-red-600 dark:text-red-400",
                      },
                      {
                        label: "Monthly profit",
                        val: inr(Number(calcResult.monthly_profit)),
                        cls: "text-blue-600 dark:text-blue-400",
                        sub: calcResult.yearly_profit
                          ? "~" + inr(Number(calcResult.yearly_profit)) + "/yr"
                          : undefined,
                      },
                      {
                        label: "Break-even",
                        val:
                          String(Number(calcResult.breakeven_units) || 0) +
                          " units",
                      ...
                      },
                      {
                        label: "ROI",
                        val:
                          calcResult.roi_pct !== undefined
                            ? pct(Number(calcResult.roi_pct))
                            : "—",
                      ...
                      },
                      {
                        label: "True ACOS",
                        val:
                          calcResult.acos_pct !== undefined
                            ? pct(Number(calcResult.acos_pct))
                            : "—",
                        cls:
                          calcResult.acos_pct !== undefined &&
                          Number(calcResult.acos_pct) > 20
                            ? "text-amber-600 dark:text-amber-400"
                    ...
                    <div
                      key={i}
                      className={`relative bg-white dark:bg-slate-900/80 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm ${m.locked ? "overflow-hidden" : ""}`}
                    >
                      ...
                      {m.locked && (
                        <div
                          className="absolute inset-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm flex items-center justify-center cursor-pointer rounded-2xl"
                          onClick={() =>
                            setUpgrade({ open: true, feature: m.label })
                          }
                        >
                    ...
                    {isBasicPlus && waterfallData.length > 0 ? (
                      <div className="space-y-2.5">
                            ...
                            >
                              ...
                              <div className="flex-1 bg-slate-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min(rowPct * 2.5, 100)}%`,
                                    background: row.color,
                                  }}
                                />
                              </div>
                              <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 w-14 text-right">
                                {inr(row.value)}
                              </span>
                        ...
                        <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                          ...
                          <span
                            className={`text-base font-black ${Number(calcResult.profit_per_unit) > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                          >
                            {inr(Number(calcResult.profit_per_unit))} (
                            {pct(Number(calcResult.net_margin_pct))})
                          </span>
                        </div>
                      </div>
                    ) : (
                      <div className="relative min-h-[100px]">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                          ...
                          <button
                            className="text-xs px-4 py-1.5 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-full shadow"
                            onClick={() =>
                              setUpgrade({
                                open: true,
                                feature: "Cost waterfall",
                              })
                            }
                          >
                            Unlock Full Access
          ...
          {tabLoading && (
            <Card className="bg-background rounded-2xl shadow-lg">
              <CardContent className="p-12 flex flex-col items-center gap-3">
                ...
                <p className="text-slate-500 text-sm">Building scenarios...</p>
              </CardContent>
            </Card>
          )}
          {!tabLoading && scenarios && (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {(scenarios as Record<string, unknown>[]).map((s, i) => (
                  <Card
                    key={i}
                    className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:shadow-md hover:scale-[1.01] transition-all"
                  >
                    <CardContent className="p-4">
                      <div
                        className="w-2 h-2 rounded-full mb-2"
                        style={{ background: String(s.color ?? "#888") }}
                      />
                      <p
                        className="text-xs font-semibold uppercase tracking-wider mb-2"
                        style={{ color: String(s.color ?? "#888") }}
                      >
                        {String(s.label ?? "")}
                      </p>
                      <p
                        className={`text-2xl font-black ${Number(s.profit_per_unit) > 0 ? "text-slate-800 dark:text-slate-100" : "text-red-600 dark:text-red-400"}`}
                      >
                        {inr(Number(s.profit_per_unit))}
                        <span className="text-xs font-normal text-slate-400 dark:text-slate-500">
                          /unit
                        </span>
                      </p>
                      <p
                        className={`text-sm font-semibold mt-1 ${Number(s.net_margin_pct) > 15 ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}
                      >
                        {pct(Number(s.net_margin_pct))} margin
                      </p>
                      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-1 text-xs text-slate-500 dark:text-slate-400">
                        <div className="flex justify-between">
                          <span>Monthly</span>
                          <span className="font-semibold text-slate-700 dark:text-slate-200">
                            {inr(Number(s.monthly_profit))}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Units</span>
                          <span className="font-semibold text-slate-700 dark:text-slate-200">
                            {Number(s.units || 0).toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>ROI</span>
                          <span className="font-semibold text-slate-700 dark:text-slate-200">
                            {pct(Number(s.roi_pct))}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>ACOS</span>
                          <span
                            className={`font-semibold ${Number(s.acos_pct) > 20 ? "text-amber-600 dark:text-amber-400" : "text-slate-700 dark:text-slate-200"}`}
                          >
                            {pct(Number(s.acos_pct))}
                          </span>
                        </div>
                      </div>
                    </CardContent>
              ...
              <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-slate-800 dark:text-slate-100">
                    Price sensitivity — margin vs selling price
                  </CardTitle>
                  <CardDescription className="dark:text-slate-400">
                    How net margin changes across a ±30% price range
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={220}>
                    ...
                    >
                      ...
                      />
                      <Tooltip
                        contentStyle={getChartStyle(isDark)}
                        formatter={(v: unknown) => [
                          Number(v).toFixed(1) + "%",
                          "Net margin",
                        ]}
                        labelFormatter={(v) => "Price: ₹" + String(v)}
                      />
                      <Line
                        type="monotone"
                        dataKey="margin_pct"
      ...
      {activeTab === "market" && (
        <div className="space-y-5">
            ...
            <Card className="bg-background rounded-2xl shadow-lg">
              <CardContent className="p-12 flex flex-col items-center gap-3">
                ...
                <p className="text-slate-500 text-sm">
                  Fetching live market data from your DB...
                </p>
              </CardContent>
            </Card>
          )}
          {!tabLoading && marketIntel && bench && (
            <>
              <div className="bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 rounded-r-2xl p-4 text-sm text-blue-800 dark:text-blue-300 font-medium">
              ...
              {marketIntel.your_price_position && (
                ...
                >
                  <Target className="w-4 h-4 shrink-0" />
                  Your price ({inr(Number(marketIntel.your_price))}) is{" "}
                  {String(marketIntel.your_price_position ?? "")} in this
                  category
                </div>
              )}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                {(
                  [
                    ...
                    { label: "Max price", val: inr(Number(bench.max_price)) },
                    {
                      label: "Avg rating",
                      val:
                        bench.avg_rating != null
                          ? "★ " + Number(bench.avg_rating).toFixed(1)
                          : "N/A",
                    },
                    {
                      label: "Avg sales/mo",
                      val:
                        bench.avg_sales_volume != null
                          ? Math.round(
                              Number(bench.avg_sales_volume),
                            ).toLocaleString()
                          : "N/A",
                    },
                    {
                      label: "MRP discount",
                      val:
                        bench.mrp_discount_depth_pct != null
                          ? Math.round(Number(bench.mrp_discount_depth_pct)) +
                            "%"
                          : "N/A",
                    },
                  ...
                  <div
                    key={i}
                    className="bg-white dark:bg-slate-900/80 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm text-center"
                  >
                    <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">
                      {m.label}
                    </p>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart
                      data={[
                        {
                          name: "Market min",
                          value: Number(bench.min_price ?? 0),
                        },
                        {
                          name: "Your price",
                          value: Number(inputs.selling_price),
                        },
                        {
                          name: "Market avg",
                          value: Number(bench.avg_price ?? 0),
                        },
                        {
                          name: "Market max",
                          value: Number(bench.max_price ?? 0),
                        },
                      ]}
                    ...
                    >
                      ...
                      <Tooltip
                        contentStyle={getChartStyle(isDark)}
                        formatter={(v: unknown) => [inr(Number(v)), "Price"]}
                      />
                      <Bar
                        dataKey="value"
                        radius={[6, 6, 0, 0]}
              ...
              {Array.isArray(marketIntel.price_bands) &&
                (marketIntel.price_bands as unknown[]).length > 0 && (
                  <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm text-slate-800 dark:text-slate-100">
                        Price band whitespace — from your DB
                      </CardTitle>
                      <CardDescription className="dark:text-slate-400">
                        Green = fewer competitors · Red = crowded · Real data
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      {(
                        marketIntel.price_bands as Record<string, unknown>[]
                      ).map((b, i) => {
                        const opp = String(b.opportunity ?? "");
                        const c =
                          opp === "High"
                        ...
                        const maxB = Math.max(
                          ...(
                            marketIntel.price_bands as Record<string, unknown>[]
                          ).map((x) => Number(x.brand_count) || 0),
                          1,
                        );
                        return (
                          <div key={i} className="flex items-center gap-3">
                            <span className="text-xs text-slate-600 dark:text-slate-300 w-40 shrink-0">
                              {String(b.band ?? "")}
                            </span>
              ...
              {Array.isArray(bench.top_brands) &&
                (bench.top_brands as unknown[]).length > 0 && (
                  <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm text-slate-800 dark:text-slate-100">
                        Top competitors in {String(inputs.category)} —{" "}
                        {String(inputs.marketplace)}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="flex flex-wrap gap-2">
                        {(bench.top_brands as string[]).map((b, i) => (
                          <span
                            key={i}
                            className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full text-xs font-medium text-slate-700 dark:text-slate-300"
                          >
                            #{i + 1} {String(b ?? "")}
                          </span>
                        ))}
      ...
      {activeTab === "health" && (
        <div className="space-y-5">
          ...
          {!tabLoading && healthData && (
            <>
              <div
                className={`relative rounded-3xl p-6 text-white overflow-hidden ${Number(healthData.overall_score) > 75 ? "bg-gradient-to-r from-emerald-500 to-green-600" : Number(healthData.overall_score) > 50 ? "bg-gradient-to-r from-blue-500 to-cyan-500" : "bg-gradient-to-r from-amber-500 to-orange-500"}`}
              >
                <div className="flex items-center gap-6">
                  <div className="shrink-0 text-center">
                    <p className="text-5xl font-black">
                      {Math.round(Number(healthData.overall_score) || 0)}
                    </p>
                    <p className="text-white/70 text-sm">/100</p>
                  </div>
                  <div>
                    ...
                    <p className="text-3xl font-black">
                      {String(healthData.overall_label ?? "")}
                    </p>
                    <p className="text-white/80 text-sm mt-1">
                      Based on margin, ACOS, returns, volume, and ROI
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex items-center gap-2 text-slate-800 dark:text-slate-100">
                      <ShieldCheck className="w-4 h-4 text-purple-500" /> Metric
                      breakdown
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {(healthData.metrics as Record<string, unknown>[]).map(
                      (m, i) => {
                        const color =
                          String(m.status) === "good"
                            ? "#10b981"
                            : String(m.status) === "warn"
                              ? "#f59e0b"
                              : "#ef4444";
                        return (
                          <div key={i}>
                            <div className="flex items-center justify-between text-sm mb-1.5">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-slate-800 dark:text-slate-100 w-8 text-right">
                                  {Math.round(Number(m.score) || 0)}
                                </span>
                              </div>
                            </div>
                ...
                <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={230}>
                      <RadarChart
                        data={(
                          healthData.metrics as Record<string, unknown>[]
                        ).map((m) => ({
                          subject: String(m.label ?? "")
                            .replace(" health", "")
                            .replace(" risk", "")
                            .replace(" momentum", ""),
                          value: Number(m.score) || 0,
                        }))}
                      >
              ...
              <Card className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2 text-slate-800 dark:text-slate-100">
                    <Zap className="w-4 h-4 text-amber-500" /> Action
                    recommendations
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {(
                    healthData.recommendations as Record<string, unknown>[]
                  ).map((r, i) => (
                    <div
                      key={i}
                    ...
                    >
                      <div
                        className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-white font-black text-sm shadow ${String(r.priority) === "high" || String(r.priority) === "critical" ? "bg-red-500" : "bg-amber-500"}`}
                      >
                        {i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span
                            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${String(r.priority) === "high" || String(r.priority) === "critical" ? "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400" : "bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400"}`}
                          >
                            {String(r.priority ?? "")}
                          </span>
                        </div>
