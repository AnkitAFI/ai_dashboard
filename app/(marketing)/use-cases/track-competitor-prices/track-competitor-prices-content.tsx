...
//               <span className="block mb-2 text-2xl sm:text-3xl md:text-4xl lg:text-5xl bg-gradient-to-r from-orange-500 via-red-500 to-orange-600 bg-clip-text text-transparent">
//                 Never Lose Sales
//               </span>
...
//             {[
//               { icon: <RefreshCw className="w-6 h-6 sm:w-6 sm:h-6" />, title: "Competitors change prices multiple times a day you can't keep up manually", color: "from-red-500 to-orange-500" },
...
//                 <div className={`w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br ${pain.color} rounded-2xl flex items-center justify-center mb-3 sm:mb-4 text-white group-hover:scale-110 transition-transform shadow-md`}>{pain.icon}</div>
...
//       {/* ══ HALF THE JOB ══ */}
//       <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-gray-50 dark:bg-gray-900">
//         <div className="max-w-6xl mx-auto">
//           <div className="text-center mb-8 sm:mb-12">
//             <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
...
//       {/* ══ HOW IT WORKS ══ */}
//       <section id="how-it-works" className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-white dark:bg-gray-950">
//         <div className="max-w-6xl mx-auto">
...
//               { step: 3, title: "Get Instant Alerts", desc: "", visual: null, bg: "", isAlerts: true },
//             ].map((step, i) => (
...
//                   <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-orange-600 to-red-600 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 text-xl sm:text-2xl font-black text-white shadow-lg">{step.step}</div>
...
//       <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-white dark:bg-gray-950">
//         <div className="max-w-6xl mx-auto">
//           <div className="text-center mb-8 sm:mb-12">
//             <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
...
//       {/* ══ ICP CTAs ══ */}
//       <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-gradient-to-br from-orange-500 via-red-500 to-orange-600">
//         <div className="max-w-4xl mx-auto text-center">
...
export default function TrackCompetitorPricesPage() {
  ...
  const DesktopDropdown = ({ label, menuKey, accent = "blue" }: { label: string; menuKey: keyof NavigationMenu; accent?: "blue" | "indigo" }) => {
    const items = navigationMenu[menuKey];
    const isActive = activeDropdown === label;
    const ac = accent === "indigo";
    return (
      <div className="relative">
        <button onMouseEnter={() => setActiveDropdown(label)} className={`px-2 xl:px-3 py-2 text-xs xl:text-sm font-medium rounded-lg transition-all flex items-center gap-1 ${isActive ? (ac ? "text-indigo-600 font-semibold" : "text-blue-600 font-semibold") : (ac ? "text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20" : "text-gray-700 dark:text-gray-300 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20")}`}>
          {label}<ChevronDown className={`w-3 h-3 xl:w-3.5 xl:h-3.5 transition-transform ${isActive ? "rotate-180" : ""}`} />
        </button>
        {isActive && (
          <div onMouseLeave={() => setActiveDropdown(null)} className="absolute top-full left-0 mt-2 w-64 xl:w-72 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-2 z-50">
            {items.map((item, i) => (
              item.route ? (
                <Link
                  ...
                  onClick={() => { setActiveDropdown(null); setIsMenuOpen(false); }}
                  className={`w-full px-4 py-2.5 flex items-center gap-3 group transition-colors ${
                    ac ? "hover:bg-indigo-50 dark:hover:bg-indigo-900/20" : "hover:bg-blue-50 dark:hover:bg-blue-900/20"
                  }`}
                >
                  <span className={`flex-shrink-0 ${ac ? "text-indigo-600 dark:text-indigo-400" : "text-blue-600 dark:text-blue-400"}`}>{item.icon}</span>
                  <span className="text-xs xl:text-sm text-gray-700 dark:text-gray-300 flex-1 text-left">{item.name}</span>
                  {item.badge && <span className="text-xs bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0">{item.badge}</span>}
                </Link>
              ) : (
                <span
                  key={i}
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background opacity-100 dark:bg-gray-900/95 backdrop-blur-none shadow-lg" : "bg-background dark:bg-gray-900/80 backdrop-blur-none"}`}>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-18 lg:h-20">
            ...
            <span className="text-lg sm:text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent ml-1 sm:ml-2">Insydz</span>
          </a>
            <div className="hidden lg:flex items-center space-x-0.5 xl:space-x-3" ref={dropdownRef}>
              <DesktopDropdown label="Solutions"  menuKey="Solutions" />
              <DesktopDropdown label="Use Cases"  menuKey="Use Cases" accent="indigo" />
              <DesktopDropdown label="Features"   menuKey="Features" />
              <Link href="/pricing" onMouseEnter={() => setActiveDropdown(null)} className="px-2 xl:px-3 py-2 text-xs xl:text-sm text-gray-700 dark:text-gray-300 hover:text-indigo-600 font-medium rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all">Pricing</Link>
              <DesktopDropdown label="Free Tools" menuKey="Free Tools" />
              <DesktopDropdown label="Compare"    menuKey="Compare" />
              <DesktopDropdown label="Resources"  menuKey="Resources" />
              <DesktopDropdown label="About"      menuKey="About" />
              <Link href="/login" onMouseEnter={() => setActiveDropdown(null)} className="ml-1 text-xs xl:text-sm bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-semibold px-4 xl:px-5 py-2 rounded-full shadow-lg hover:shadow-xl transition-all transform hover:scale-105">Login</Link>
              <button className="ml-1 p-1.5 xl:p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors flex-shrink-0" onClick={() => setIsDarkMode(!isDarkMode)} aria-label="Toggle dark mode">
                {isDarkMode ? <Sun className="w-4 h-4 xl:w-5 xl:h-5 text-yellow-400" /> : <Moon className="w-4 h-4 xl:w-5 xl:h-5 text-gray-800" />}
              </button>
            </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 max-h-[calc(100vh-4rem)] overflow-y-auto">
            <div className="px-3 sm:px-4 py-3 sm:py-4 space-y-1.5">
              <a href="/" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 w-full px-3 sm:px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg font-medium text-sm">
                <ArrowLeft className="w-4 h-4 flex-shrink-0" /> Back to Home
              </a>
              {(["Solutions","Use Cases","Features","Free Tools","Compare","Resources","About"] as (keyof NavigationMenu)[]).map((key) => (
                <div key={key}>
                  <button onClick={() => toggleMobileMenu(key)} className={`flex items-center justify-between w-full px-3 sm:px-4 py-2 rounded-lg font-medium text-sm ${key === "Use Cases" ? "text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 font-semibold" : "text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20"}`}>
                    {key}<ChevronDown className={`w-4 h-4 transition-transform flex-shrink-0 ${mobileActiveMenu === key ? "rotate-180" : ""}`} />
                  </button>
                  {mobileActiveMenu === key && (
                    <div className="ml-3 sm:ml-4 mt-0.5 space-y-0.5">
                      {(navigationMenu[key] as MenuItemWithBadge[]).map((item, i) => (
                        item.route ? (
                          <Link key={i} href={item.route} onClick={() => setIsMenuOpen(false)} className={`flex items-center gap-2 w-full px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400 rounded-lg ${key === 'Use Cases' ? 'hover:bg-indigo-50 dark:hover:bg-indigo-900/20' : 'hover:bg-blue-50 dark:hover:bg-blue-900/20'}`}>
                            <span className="flex-shrink-0">{item.icon}</span>
                            <span className="text-left flex-1">{item.name}</span>
              ...
              <a href="/login" onClick={() => setIsMenuOpen(false)} className="w-full mt-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-center text-sm py-2 rounded-lg font-semibold block">Login</a>
            </div>
          </div>
        )}
      </nav>
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 items-center">
            <div className="space-y-5 sm:space-y-6 lg:space-y-8">
              ...
              <p className="text-base sm:text-sm lg:text-lg text-gray-700 dark:text-gray-300 leading-relaxed max-w-xl">
                India's most powerful <strong>competitor price tracking tool</strong> for Amazon and Flipkart sellers. Monitor rival pricing automatically and react instantly
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                <Button onClick={() => document.getElementById("how-it-works")?.scrollIntoView({ behavior: "smooth" })} size="lg" variant="outline" className="border-2 border-blue-600 text-blue-700 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 font-semibold px-6 sm:px-8 py-5 sm:py-6 text-base sm:text-lg rounded-full">
                  See How It Works
                </Button>
              </div>
            </div>
            ...
            <div className="relative mt-4 lg:mt-0">
              <div className="relative bg-white dark:bg-gray-900 border-2 border-blue-200 dark:border-blue-800 rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8 shadow-2xl">
                ...
                <div className="absolute -top-3 sm:-top-4 -right-3 sm:-right-4 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl sm:rounded-2xl px-3 sm:px-4 py-1.5 sm:py-2 shadow-xl">
                  <p className="text-white font-bold text-xs sm:text-sm">Live Tracking</p>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
              Why Manual Price Tracking
              <br />
              <span className="text-blue-600">Is Killing Your Profits</span>
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto leading-relaxed">
              Most Amazon and Flipkart sellers still check competitor prices by hand. Here's why that approach fails, every time.
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-12">
            {[
              { icon: <RefreshCw className="w-6 h-6 sm:w-6 sm:h-6" />, title: "Competitors change prices multiple times a day you can't keep up manually", color: "from-blue-500 to-indigo-500" },
              { icon: <Clock className="w-6 h-6 sm:w-8 sm:h-8" />, title: "You notice price drops too late after your Buy Box rank has already fallen", color: "from-indigo-500 to-blue-400" },
              { icon: <FileSpreadsheet className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Excel tracking is outdated within hours the data is stale before you use it", color: "from-blue-400 to-indigo-500" },
              { icon: <TrendingDown className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Price wars silently eat your margins panic discounting costs more than lost sale", color: "from-indigo-500 to-blue-600" },
            ].map((pain, i) => (
          ...
          <div className="bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/20 dark:to-indigo-900/20 border-2 border-blue-400 dark:border-blue-600 rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-center shadow-lg mb-8 sm:mb-12">
            <p className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white mb-2">
              Most sellers lose <span className="text-blue-600">20–40% of potential revenue annually</span>
            </p>
            <p className="text-gray-700 dark:text-gray-300 text-sm sm:text-lg">
              because they react late to competitor price changes or don't react at all until the damage is  <span className="text-blue-600">already visible in their sales report.</span>
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-5 sm:gap-8">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/10 border-2 border-blue-300 dark:border-blue-700 rounded-2xl p-6 sm:p-8">
              <div className="flex items-center gap-3 mb-4 sm:mb-6">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-500 rounded-full flex items-center justify-center flex-shrink-0"><Check className="w-5 h-5 sm:w-6 sm:h-6 text-white" /></div>
                <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">With Insydz</h3>
              </div>
              <ul className="space-y-2 sm:space-y-3">
                {["Automatic 24/7 competitor monitoring no manual checks needed","Instant WhatsApp alerts when any competitor changes price","Never miss a price change even at 2am during Big Billion Days","AI shows your margin floor before you reprice","Set it once monitor forever"].map((item, i) => (
                  <li key={i} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700 dark:text-gray-300">
                    <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-600 flex-shrink-0 mt-0.5" />{item}
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
              Tracking Prices Is Only Half the Job.
              <br />
              <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">The Other Half Is Knowing How to React.</span>
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto leading-relaxed">
              Most competitor price tracking tools give you a spreadsheet of price changes. Insydz gives you the price change and the recommended response with your margin floor already calculated.
            </p>
          ...
          <div className="grid sm:grid-cols-2 gap-5 sm:gap-8">
            {[
              { icon: <Eye className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Real-Time Price Monitoring", desc: "Insydz tracks competitor pricing across Amazon India and Flipkart continuously. Price changes are detected within minutes not at the end of the day when you check your dashboard.", link: "/features/price-tracker", linkLabel: "marketplace price tracking dashboard", color: "from-blue-500 to-cyan-500" },
              { icon: <BarChart3 className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Historical Price Trends", desc: "See how competitor prices have moved over the last 30, 60, or 90 days. Spot recurring pricing patterns like weekend flash drops or pre-sale inflation before they catch you off guard again.", link: "/features/price-history", linkLabel: "historical price tracker", color: "from-indigo-500 to-blue-500" },
              { icon: <Shield className="w-6 h-6 sm:w-8 sm:h-8" />, title: "Buy Box Risk Alerts", desc: "Insydz monitors Buy Box eligibility signals in real time. When a competitor's price drops close to your threshold, you get an alert before you lose the Buy Box not after your sales velocity has already dropped.", link: "/features/buy-box-alerts", linkLabel: "buy box monitoring software", color: "from-blue-600 to-indigo-500" },
              { icon: <Bell className="w-6 h-6 sm:w-8 sm:h-8" />, title: "WhatsApp Notifications", desc: "Indian sellers don't monitor email dashboards. Insydz delivers every critical price alert directly to your WhatsApp with the competitor name, old price, new price, percentage drop, and an AI-suggested response price in INR.", link: "/features/whatsapp-alerts-feature", linkLabel: "WhatsApp price alerts", color: "from-indigo-500 to-blue-400" },
            ].map((item, i) => (
              <div key={i} className="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-2xl p-6 sm:p-8 hover:border-blue-400 hover:shadow-xl transition-all">
                <div className="flex items-start gap-3 sm:gap-4">
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br ${item.color} rounded-xl flex items-center justify-center text-white shadow-lg flex-shrink-0`}>{item.icon}</div>
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2">{item.title}</h3>
                    <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm leading-relaxed mb-2 sm:mb-3">{item.desc}</p>
                    <button onClick={() => router.push(item.link)} className="text-xs font-semibold text-blue-600 hover:text-blue-700 underline">See {item.linkLabel}</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-4 sm:mb-6 text-gray-900 dark:text-white">
              Competitor Price Tracking
              <br />
              <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">Done Automatically</span>
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
              Insydz continuously tracks competitor prices across Amazon & Flipkart and alerts you the moment something changes.
            </p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-12 relative">
            ...
            {[
              { step: 1, title: "Add Your Product", desc: "Enter your ASIN or Flipkart listing. Insydz automatically identifies 100+ competitors in your category. Setup: under 2 minutes.", visual: <ShoppingBag className="w-6 h-6 sm:w-6 sm:h-6 text-blue-600 mx-auto" />, bg: "bg-blue-100 dark:bg-blue-900/20", isAlerts: false },
              { step: 2, title: "AI Monitors Prices 24/7", desc: "We track competitor price changes, discounts, and stock signals continuously overnight, weekends, and during festive sale events.", visual: <Eye className="w-6 h-6 sm:w-6 sm:h-6 text-indigo-600 mx-auto animate-pulse" />, bg: "bg-indigo-100 dark:bg-indigo-900/20", isAlerts: false },
              { step: 3, title: "Get Instant Alerts", desc: "", visual: null, bg: "", isAlerts: true },
            ].map((step, i) => (
              <div key={i} className="relative z-10">
                <div className="bg-white dark:bg-gray-800 border-2 border-blue-300 dark:border-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-center shadow-xl hover:shadow-2xl transition-all">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 text-xl sm:text-2xl font-black text-white shadow-lg">{step.step}</div>
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">{step.title}</h3>
                  {step.isAlerts ? (
                    <div className="space-y-2 sm:space-y-3 text-left">
                    {[
                      { cls: "bg-red-50 dark:bg-red-900/20 border-red-300 dark:border-red-700", icon: "text-red-600", text: 'Competitor dropped price by 11%' },
                      { cls: "bg-blue-50 dark:bg-blue-900/20 border-blue-300 dark:border-blue-700", icon: "text-blue-600", text: 'Lowest price changed Buy Box at risk' },
                      { cls: "bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700", icon: "text-green-600", text: 'AI suggests: ₹1,049 protects margin & Buy Box' },
                    ].map((a, ai) => (
                      <div
                        key={ai}
                        className={`flex items-center gap-2 ${a.cls} border rounded-lg p-2.5 sm:p-3`}
                      >
                        <br /><CheckCircle2
                          className={`w-4 h-4 sm:w-5 sm:h-5 ${a.icon} flex-shrink-0`}
                        />
                        <span className="text-xs sm:text-sm text-gray-800 dark:text-gray-300 whitespace-nowrap overflow-hidden text-ellipsis">
                          {a.text}
                        </span>
                      </div>
                    ))}
          ...
          <div className="text-center mt-8 sm:mt-12">
            <Button onClick={handleGetStarted} size="lg" className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold px-8 sm:px-12 py-5 sm:py-6 text-base sm:text-lg rounded-full shadow-2xl hover:shadow-blue-500/50 transition-all group">
              Track Your First Competitor Free
              <ChevronRight className="ml-2 group-hover:translate-x-1 transition-transform" />
            </Button>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
              What Sellers Do with
              <br />
              <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">Competitor Price Insights</span>
            </h2>
            <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
              Real-time pricing intelligence is only valuable if it drives action. Here's what Indian sellers actually do once they have it.
            </p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {[
              ...
              <div key={i} className="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl p-5 sm:p-6 hover:border-blue-400 hover:shadow-lg transition-all">
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 dark:bg-blue-900/20 rounded-xl flex items-center justify-center text-blue-600 dark:text-blue-400">{uc.icon}</div>
                </div>
                <h3 className="font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2 text-sm sm:text-base">{uc.title}</h3>
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{uc.desc}</p>
              </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
              How Insydz Is Different
              <br />
              <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">Built for Indian Marketplace Sellers</span>
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
              Global price tracking tools weren't designed for the way Indian marketplaces work. Insydz is built specifically for Amazon.in and Flipkart with Indian seller behaviour, INR data, and WhatsApp-first alerts at its core.
            </p>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-2 border-blue-300 dark:border-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-8 mb-8 sm:mb-12 shadow-lg">
            ...
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-3 sm:mb-4 text-sm sm:text-base">Ravi sells premium non-stick cookware on Amazon India. Every Monday his sales would inexplicably drop but by Tuesday, they were back. For months, he assumed it was an algorithm issue.</p>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-3 sm:mb-4 text-sm sm:text-base">When he started using Insydz, the pattern revealed itself immediately: a competitor was running a Sunday-night-to-Monday-morning flash price drop going from ₹1,299 to ₹899 at 10pm on Sundays and reverting by Tuesday morning.</p>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4 sm:mb-6 text-sm sm:text-base">With Insydz, he set a price alert for that competitor. The next Sunday at 10:07 PM, his WhatsApp buzzed. He repriced from his phone in 3 minutes. By Monday morning, his Buy Box was secure.</p>
            <div className="grid grid-cols-3 gap-2 sm:gap-4">
              {[{ label: "Saved in First Month", value: "₹18,000" },{ label: "Response Time", value: "3 minutes" },{ label: "Alert Received", value: "10:07 PM Sunday" }].map((stat, i) => (
            ...
            <div className="min-w-[480px]">
              <div className="grid grid-cols-3">
                ...
                {comparisonRows.map((row, i) => (
                  <>
                    <div key={`f-${i}`} className={`px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 dark:border-gray-800 ${i % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50 dark:bg-gray-800/50"}`}><p className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 font-medium">{row.feature}</p></div>
                    <div key={`m-${i}`} className={`px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 dark:border-gray-800 text-center ${i % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50 dark:bg-gray-800/50"}`}><p className="text-xs sm:text-sm text-gray-500 flex items-center justify-left gap-1"><X className="w-3 h-3 text-red-500 flex-shrink-0" />{row.manual}</p></div>
                    <div key={`s-${i}`} className={`px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 dark:border-gray-800 ${i % 2 === 0 ? "bg-blue-50 dark:bg-blue-900/10" : "bg-blue-50/50 dark:bg-blue-900/10"}`}><p className="text-xs sm:text-sm text-blue-700 dark:text-blue-400 font-semibold flex items-center justify-left gap-1"><Check className="w-3 h-3 text-green-600 flex-shrink-0" />{row.insydz}</p></div>
                  </>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
              What Real-Time Price Tracking
              <br />
              <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">Is Worth to Your Business</span>
            </h2>
            <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
              Two sellers in the same Flipkart category. Same products. Same pricing. The only difference: one has Insydz, one doesn't.
            </p>
          ...
          <div className="grid sm:grid-cols-2 gap-5 sm:gap-8 mb-6 sm:mb-8">
            {[
              { title: "Without Insydz Monthly Cost", rows: roiWithout, total: "Total Monthly Cost of Slow Reactions", totalValue: "−₹42,000", headerCls: "bg-red-50 dark:bg-red-900/30", textCls: "text-red-700 dark:text-red-400", valueCls: "text-red-600", totalBg: "bg-red-50 dark:bg-red-900/20", totalTextCls: "text-red-700", borderCls: "border-red-300 dark:border-red-700" },
              { title: "With Insydz Same Month", rows: roiWith, total: "Net Monthly Value with Insydz", totalValue: "+₹51,700", headerCls: "bg-blue-50 dark:bg-blue-900/30", textCls: "text-blue-700 dark:text-blue-400", valueCls: "text-blue-600", totalBg: "bg-blue-50 dark:bg-blue-900/20", totalTextCls: "text-blue-700", borderCls: "border-blue-300 dark:border-blue-700" },
            ].map((panel, pi) => (
              <div key={pi} className={`rounded-2xl border-2 ${panel.borderCls} overflow-hidden shadow-lg`}>
                <div className={`${panel.headerCls} px-4 sm:px-6 py-3 sm:py-4`}><p className={`font-bold ${panel.textCls} text-sm sm:text-lg`}>{panel.title}</p></div>
                <div className="bg-white dark:bg-gray-900">
      ...
      <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-white dark:bg-gray-950">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-3 sm:mb-4 text-gray-900 dark:text-white">
            Start Free.
            <br />
            <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">See Real Price Movements</span>
          </h2>
          <p className="text-base sm:text-xl text-gray-600 dark:text-gray-400 mb-8 sm:mb-10">Free Plan — ₹0 / Forever No credit card required</p>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-2 border-blue-300 dark:border-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-8 mb-6 sm:mb-8 text-left shadow-xl">
            ...
            <div className="bg-white dark:bg-gray-900 border border-blue-200 dark:border-blue-700 rounded-xl p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                <span className="font-bold text-blue-600">Upgrade teaser:</span> Paid plans unlock deeper tracking, more competitors per product, 90-day historical price trends, advanced Buy Box monitoring, and AI pricing recommendations.
              </p>
            </div>
          </div>

          <Button onClick={handleGetStarted} size="lg" className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold px-8 sm:px-12 py-5 sm:py-6 text-base sm:text-lg rounded-full shadow-2xl group">
            Start Free Price Tracking
          </Button>
        </div>
      </section>
      ...
      <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-8 sm:mb-12 text-center text-gray-900 dark:text-white">
            Who Should Use
            <br />
            <span className="bg-gradient-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent">Competitor Price Tracking?</span>
          </h2>
          <div className="grid sm:grid-cols-2 gap-5 sm:gap-8">
            <div className="bg-blue-50 dark:bg-blue-900/10 border-2 border-blue-300 dark:border-blue-700 rounded-2xl p-6 sm:p-8">
              <div className="flex items-center gap-3 mb-4 sm:mb-6">
      ...
      <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-white dark:bg-gray-950">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3 sm:space-y-4">
            {faqs.map((faq) => (
              <div key={faq.id} className="bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden hover:border-blue-300 transition-all">
                <button onClick={() => setExpandedFaq(expandedFaq === faq.id ? null : faq.id)} className="w-full px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between text-left hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors">
                  <span className="font-bold text-gray-900 dark:text-white pr-3 sm:pr-4 text-sm sm:text-base lg:text-lg">{faq.q}</span>
      ...
      {/* ══ ICP CTAs ══ */}
      <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-700">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-2 sm:mb-4 text-white">Three Paths </h2>
          <p className="text-base sm:text-xl text-white/90 mb-8 sm:mb-12">Based on Where You Are Now</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10">
            {[
              ...
              <div key={i} className="bg-white/10 backdrop-blur-none border border-white/20 rounded-2xl p-5 sm:p-6 text-left">
                <p className="font-bold text-white mb-1.5 sm:mb-2 text-sm sm:text-base">{card.label}</p>
                <p className="text-white/80 text-xs sm:text-sm mb-3 sm:mb-4">{card.desc}</p>
                <button onClick={card.action} className="text-blue-100 font-semibold text-xs sm:text-sm hover:text-white transition-colors underline">{card.cta}</button>
              </div>
            ))}
          </div>
      ...
      <section className="pt-8 pb-8 sm:pt-10 sm:pb-12 lg:pt-12 lg:pb-14 px-4 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              { title: "Find Profitable Products", icon: <Target className="w-6 h-6 sm:w-8 sm:h-8" />, color: "from-blue-500 to-cyan-500", route: "/use-cases/find-profitable-products" },
              { title: "Analyse Customer Reviews", icon: <MessageCircle className="w-6 h-6 sm:w-8 sm:h-8" />, color: "from-indigo-500 to-blue-500", route: "/use-cases/analyze-customer-reviews" },
              { title: "Improve Amazon & Flipkart SEO", icon: <Search className="w-6 h-6 sm:w-8 sm:h-8" />, color: "from-blue-500 to-indigo-600", route: "/use-cases/improve-seo" },
              { title: "Avoid Stockouts & Missed Sales", icon: <Package className="w-6 h-6 sm:w-8 sm:h-8" />, color: "from-indigo-500 to-blue-700", route: "/use-cases/avoid-stockouts" },
            ].map((uc, i) => (
              <div key={i} onClick={() => router.push(uc.route)} className="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-6 hover:border-blue-400 hover:shadow-lg transition-all cursor-pointer group">
                <div className={`w-10 h-10 sm:w-14 sm:h-14 bg-gradient-to-br ${uc.color} rounded-xl flex items-center justify-center mb-3 sm:mb-4 text-white group-hover:scale-110 transition-transform shadow-md`}>{uc.icon}</div>
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 transition-colors">{uc.title}</h3>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t-2 border-blue-300 dark:border-blue-700 p-3 sm:p-4 shadow-2xl z-40" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}>
        <Button onClick={handleGetStarted} className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-3.5 sm:py-4 rounded-full shadow-xl text-sm sm:text-base">
         Start Free Price Tracking
        </Button>
      </div>
