...
Fixes applied vs v1:
  1. Sales volume de-scaling  — Amazon stores values × 1,000,000 (e.g. "100+" → 100_000_000).
     All SV queries now divide by SV_SCALE_AMAZON / SV_SCALE_FLIPKART to recover real units.
  2. Double-division removed  — overview helpers no longer divide AVG() by product_count again.
  3. SQL injection fixed       — INTERVAL built with int() cast, never raw string interpolation.
  4. Input validation          — source validated as Literal, base_cost > 0 enforced, category_name stripped.
  5. Flipkart price columns    — product_price/min_price/max_price/avg_sales_volume are NUMERIC in DB,
     NOT text. All _FK_* expressions now use direct NUMERIC casts — no REGEXP_REPLACE needed.
  6. Expired subscriptions     — require_tier now checks subscription_expires_at.
  7. ZeroDivisionError guards  — all Python-side divisions guarded with "or 1" / explicit checks.
  8. NULL sales volume guard   — AND avg_sales_volume > 0 added to all SV queries.
  9. Launch window smoothed    — 3-week rolling average + sample_size guard before scoring.
 10. Error boundaries          — all DB helpers wrapped in try/except + db.rollback() to clear
     failed transaction state, preventing "InFailedSqlTransaction" cascade errors.
 11. user_id via header        — get_current_user_id() reads X-User-Id header (wire up JWT later).
"""
...
from fastapi import APIRouter, Depends, Header, HTTPException, Query, Cookie
from fastapi.responses import StreamingResponse
...
# ─────────────────────────────────────────────────────────────────────────────
# Scale constants
# Amazon  ingestion stores "100+" as 100 * 1_000_000 = 100_000_000
# Flipkart ingestion stores "2.5M+" as 2_500_000 (already real units)
# Adjust these if your ingestion script changes.
# ─────────────────────────────────────────────────────────────────────────────
SV_SCALE_AMAZON   = 1_000_000   # divide stored value by this → real monthly units
SV_SCALE_FLIPKART = 1           # already real units

# ─────────────────────────────────────────────────────────────────────────────
# Indian festive calendar
# ─────────────────────────────────────────────────────────────────────────────

MOVING_FESTIVALS: dict[str, dict[int, tuple]] = {
    "Holi": {
        2025: (3, 13, 15),
        2026: (3, 3, 5),
    },
...
# Note: We use get_current_user_id from deps.py for required auth,
# and a local optional helper for routes that allow anonymous access.

def get_optional_user_id(
    session_id: str = Cookie(None),
) -> Optional[str]:
    """
    Optional user identity. Returns None if not authenticated.
    Validates the session cookie if present.
    """
    if not session_id:
        return None
...
def _check_tier(user_id: Optional[str], required: str, db: Session) -> None:
    """
    Gate access by subscription tier AND expiry date.
    Raises 403 with a structured payload on failure.
    """
    if not user_id:
        raise HTTPException(
            status_code=401,
    ...
    try:
        # require_tier raises PermissionError if tier is insufficient
        require_tier(user_id, required, db)
    except PermissionError:
        raise HTTPException(
            status_code=403,
...
# ─────────────────────────────────────────────────────────────────────────────
# Tier helper — reads subscription_tier directly from users table.
# Used everywhere in this router instead of trusting the frontend's claim.
# ─────────────────────────────────────────────────────────────────────────────
...
def _get_user_tier(user_id: Optional[str], db: Session) -> str:
    """
    Return the user's current subscription_tier from the users table.
    Checks subscription_expires_at — if expired, treats as 'free'.
    Returns 'free' for anonymous / unknown users.
    """
    if not user_id:
        return "free"
    try:
...
def _has_tier(user_tier: str, required: str) -> bool:
    """True if user_tier meets or exceeds required tier."""
...
def _norm_sv(raw: Optional[float], scale: int) -> float:
    """Convert stored SV back to real monthly units."""
...
# All price/SV columns in rapidapi_flipkart_products are stored as NUMERIC
# (confirmed from DB schema). Direct NULLIF + FLOAT cast — no REGEXP_REPLACE.
# ─────────────────────────────────────────────────────────────────────────────

_FK_PRICE = "NULLIF(product_price, 0)::FLOAT"
_FK_MIN   = "NULLIF(min_price, 0)::FLOAT"
_FK_MAX   = "NULLIF(max_price, 0)::FLOAT"
_FK_SV    = "NULLIF(avg_sales_volume, 0)::FLOAT"
    ...
    return [
        {
            ...
            "avg_price":        round(float(r["avg_price"] or 0), 0),
            # Divide by scale to get real monthly units, apply 0.85 confidence discount
...
# ─────────────────────────────────────────────────────────────────────────────
# DB helpers — price trend (90-day weekly)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# DB helpers — velocity (all categories)
# ─────────────────────────────────────────────────────────────────────────────
...
def _launch_window(category_name: str, source: str, db: Session) -> dict:
    ...
    try:
        if source == "flipkart":
            rows = db.execute(
                text(f"""
                    ...
                    WHERE category_name = :cat
                      AND {_FK_PRICE} IS NOT NULL
                      AND created_at >= NOW() - INTERVAL '{int(90)} days'
                    GROUP BY DATE_TRUNC('week', created_at)
                    ORDER BY week ASC
                """),
                {"cat": category_name},
        ...
        return {"optimal_week": None, "recommendation": "Data unavailable.", "price_trend": [], "weeks_available": 0}

    if len(rows) < 3:
        return {
            "optimal_week":    None,
            "recommendation":  "Insufficient historical data for launch window calculation. Check back after more weekly snapshots are collected.",
            "price_trend":     [],
    ...
    # 3-week rolling average to smooth scraper-spike outliers
    smoothed: list[dict] = []
    for i in range(1, len(raw_trend) - 1):
        # Skip weeks with too few data points (scraper may have run only once)
        if raw_trend[i]["sample_size"] < 3:
            continue
        smoothed.append({
            "week":      raw_trend[i]["week"],
            "avg_price": round((raw_trend[i-1]["avg_price"] + raw_trend[i]["avg_price"] + raw_trend[i+1]["avg_price"]) / 3, 2),
            "avg_sv":    round((raw_trend[i-1]["avg_sv"]    + raw_trend[i]["avg_sv"]    + raw_trend[i+1]["avg_sv"])    / 3, 0),
        })

    if len(smoothed) < 2:
        # Fall back to raw if smoothing leaves too few points
        smoothed = raw_trend

    # Find week with highest combined price + volume upward slope
    best_week  = None
    best_score = -9999.0
    for i in range(1, len(smoothed)):
        price_delta = smoothed[i]["avg_price"] - smoothed[i-1]["avg_price"]
        sv_delta    = smoothed[i]["avg_sv"]    - smoothed[i-1]["avg_sv"]
        # Normalise sv_delta: divide by 1000 so it's on a similar scale to ₹ price delta
    ...
    if best_week:
        delta_days = (date.fromisoformat(best_week) - date.today()).days
        if delta_days <= 0:
            rec = "The optimal window has passed — list NOW before the next festive peak to capture residual demand."
        elif delta_days <= 7:
        ...
        rec = "No clear price inflection detected in this period. List any time and monitor weekly."
...
def _margin_sim(category_name: str, source: str, base_cost: float, db: Session) -> dict:
    ...
    # Pick the highest-priced viable scenario as recommended
    recommended = next(
        (s for s in reversed(scenarios) if s["viable"]),
        scenarios[0],
    )
...
Price + sales trend (last 8 weeks, units = estimated monthly sales benchmark):
{trend_str}
...
# ─────────────────────────────────────────────────────────────────────────────
# Shared param validation helpers
# ─────────────────────────────────────────────────────────────────────────────

def _validate_source(source: str) -> str:
    s = source.lower().strip()
    if s not in ("amazon", "flipkart"):
        raise HTTPException(status_code=422, detail={"error": "invalid_source", "allowed": ["amazon", "flipkart"]})
    return s
...
def festive_calendar(year: Optional[int] = Query(None, ge=2020, le=2035)):
    """
    FREE — Indian festive calendar for the given year (default: current year).
    No DB query — fully derived from hardcoded + enriched calendar.
    """
    y      = year or date.today().year
    events = _enrich_calendar(y)
    if date.today().month >= 10:
...
def tier_info(
    ...
    Frontend uses this to gate UI features without trusting the auth context field name.
    """
...
def festive_overview(
    ...
    """
    FREE — Top 3 trending categories by sales velocity.
    Locking is based on the user's ACTUAL tier from the users table —
    basic and premium users see all 3 categories unlocked.
    """
    source = _validate_source(source)

    # Get real tier from DB — never trust the frontend
    user_tier = _get_user_tier(user_id, db)
    unlocked  = _has_tier(user_tier, "basic")  # basic+ sees all 3

    cats = _flipkart_category_overview(db) if source == "flipkart" else _amazon_category_overview(db)
    top3 = cats[:3]

    for i, cat in enumerate(top3):
        # Index 0 is always free. Index 1-2 require basic+.
...
async def ai_forecast(
    ...
    """
    PREMIUM · SSE — AI festive surge forecast via Ollama.
    Streams: forecast / pricing strategy / stock prep / timing.
    """
    _check_tier(user_id, "premium", db)
    await _check_ollama()
...
@router.get("/ai/status")
async def ai_status():
    """Health check for Ollama — mirrors seller optimizer pattern."""
    running = await ollama_is_running()
    return {
...
def list_categories(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail={"error": "db_error", "message": "Could not fetch categories"})
