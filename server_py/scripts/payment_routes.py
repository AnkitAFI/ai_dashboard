...
def _active_order(user_id: int, db: Session) -> Optional["models.PaymentOrder"]:
    """Most recent unexpired paid order for this user."""
    return (
        db.query(models.PaymentOrder)
        .filter(
            models.PaymentOrder.user_id    == user_id,
            models.PaymentOrder.status     == "paid",
            models.PaymentOrder.expires_at >  datetime.now(),
        )
        .order_by(models.PaymentOrder.expires_at.desc())
        .first()
    )
...
def _build_invoice_pdf(order: "models.PaymentOrder") -> bytes:
...
def verify_payment(
    ...
    order = (
        db.query(models.PaymentOrder)
        .filter(models.PaymentOrder.id      == data.order_db_id,
...
def download_invoice(
    ...
    order = (
        db.query(models.PaymentOrder)
        .filter(models.PaymentOrder.id      == order_id,
                models.PaymentOrder.user_id == current_user.id,
                models.PaymentOrder.status  == "paid")
        .first()
    )
...
def payment_history(
    ...
    orders = (
        db.query(models.PaymentOrder)
        .filter(models.PaymentOrder.user_id == user_id)
        .order_by(models.PaymentOrder.created_at.desc())
        .limit(20).all()
    )
    return [{
...
async def razorpay_webhook(
    ...
    order = (db.query(models.PaymentOrder)
               .filter(models.PaymentOrder.razorpay_order_id == rzp_oid).first())
    if not order:
